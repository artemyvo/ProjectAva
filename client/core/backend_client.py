...
class BackendClient:
    ...
    # Opening-handshake deadline. A healthy server whose asyncio loop is starved by an
    # idle job (the assoc feed rebuild, a reflection pass) answers the upgrade late, not
    # never; see connect(). Bounded so a peer that accepts TCP and then hangs still fails.
    CONNECT_OPEN_TIMEOUT_S = 300.0
    # TCP-level connect (see _open_socket): how long to keep re-resolving the host and
    # retrying its addresses when every one refuses, and the per-address connect cap.
    CONNECT_RESOLVE_RETRY_S = 12.0
    CONNECT_TCP_TIMEOUT_S = 5.0

    def __init__(self) -> None:
        self._ws = None
        # The address that last accepted a connection: tried first next time, so a
        # resolver that also hands out stale addresses costs nothing on reconnect.
        self._last_good_addr: Optional[str] = None
    ...
    def connect(self, url: str) -> None:
        ...
        #
        # open_timeout: the same starvation hits the opening handshake (2026-09-11). The
        # server's assoc idle jobs (a feed rebuild is minutes of GIL-holding Python, and
        # it fires 5 min into idle) leave its loop unable to answer the HTTP upgrade
        # inside websockets' default 10 s, so connecting to a busy-but-healthy server
        # failed with "timed out during opening handshake". The server side is unbounded
        # (serve(open_timeout=None)); this side stays bounded so a peer that accepted the
        # TCP connection and then hung still fails — but generously, since ConnectWorker
        # runs off the GUI thread and the Chat tab already shows "Connecting…". A server
        # that is not listening at all is refused by TCP at once, regardless of this.
        #
        # The TCP connect is ours (`sock=`), not websockets' `socket.create_connection`
        # (2026-09-11): see _open_socket for the stale-mDNS failure it exists for.
        sock = self._open_socket(url)
        self._ws = ws_connect(url, sock=sock, max_size=None, ping_timeout=None,
    ...
    def _open_socket(self, url: str) -> socket.socket:
        """Resolve the URL's host and return a connected TCP socket, trying EVERY
        address the resolver offers — IPv4 before IPv6 (the server binds 0.0.0.0), the
        address that worked last time before either — and, when all of them refuse,
        re-resolving and retrying for CONNECT_RESOLVE_RETRY_S before giving up.

        Why (2026-09-11): the operator's client kept failing with "[Errno 61]
        Connection refused" for minutes while the server was demonstrably up and
        busy. `<server>.local` is resolved by mDNS, and macOS returns EXPIRED cache
        entries first (its optimistic-answer behaviour) — here an IPv6 link-local the
        IPv4-only server never listens on, and 192.0.2.20, an address the box no
        longer has and another machine now does — with the fresh, correct
        192.0.2.10 arriving ~0.7 s later. A lookup that lands in that window sees
        only refusing addresses; `socket.create_connection` tries them, raises the last
        error, and every quick retry does the same. A refused connect is instant, so
        trying the stale addresses costs nothing; what fixes it is coming back for the
        fresh answer. A genuinely dead server still fails, after the retry window,
        with the same refusal — and with every address it tried in the message."""
        u = urlparse(url)
        host = u.hostname or ""
        port = u.port or (443 if u.scheme == "wss" else 80)
        deadline = time.monotonic() + self.CONNECT_RESOLVE_RETRY_S
        ...
        while True:
            try:
                ...
                infos, last_err = [], e
            infos.sort(key=lambda i: (i[4][0] != self._last_good_addr, i[0] != socket.AF_INET))
            for fam, st, pr, _cn, sa in infos:
                s = socket.socket(fam, st, pr)
                s.settimeout(self.CONNECT_TCP_TIMEOUT_S)
                try:
                    s.connect(sa)
                    s.settimeout(None)
                    self._last_good_addr = sa[0]
                    return s
                except OSError as e:
                    last_err = e
                    if sa[0] not in tried:
                        tried.append(sa[0])
                    s.close()
            if time.monotonic() >= deadline:
                detail = f"{host}:{port} — tried {', '.join(tried) or 'no address'}"
                if last_err is None:
                    raise OSError(f"could not resolve {detail}")
                raise OSError(f"{last_err} ({detail}, over {int(self.CONNECT_RESOLVE_RETRY_S)} s)") from last_err
            time.sleep(0.75)
        ...
        try:
            while self._ws is not None:
                ...
                if t == "loaded":
                    ...
                    # The server's unsolicited `loaded` (its startup load landing after
                    # we connected) also names the precision and windows it loaded at;
                    # keep them so ChatWidget.sync_model_state adopts the real values.
                    for k in ("base_quant", "context_length", "reflect_context_length"):
                        if msg.get(k) is not None:
                            self.last_status[k] = msg[k]
                elif t == "unloaded":

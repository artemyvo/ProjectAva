...
class MainWindow(QMainWindow):
    ...
    def _update_status_bar(self):
        # A model that finished loading after we connected (the server listens before
        # it loads), or was unloaded elsewhere: let the Chat tab follow the receiver.
        try:
            self.chat_widget.sync_model_state()
        except Exception:
            pass
        ...
        try:
            ...
            elif getattr(cw, "server_loading", False):
                self._model_label.setText("remote · loading model…")
            else:

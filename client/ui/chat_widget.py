...
class ChatWidget(QWidget):
    ...
    def __init__(self, parent=None, server_url: Optional[str] = None):
        ...
        self.server_loading = False   # the server's startup load is in flight
    ...
    def _on_connect_done(self, result: dict) -> None:
        ...
        # The server listens before it loads its startup model (2026-09-11): a connect
        # during that window is "loading", not "no model" — the model lands later and
        # sync_model_state() unlocks the tab when it does.
        self.server_loading = bool(result.get("loading", False))
        ...
        if self.model_loaded:
            ...
            if self.server_loading:
                self.txt_chat_log.append("Connected — the server is still loading its model; "
                                         "chat unlocks when it finishes.\n")
            else:
    ...
    def sync_model_state(self) -> None:
        """Adopt a model-state change that happened AFTER this client connected: the
        startup load landing (the server pushes `loaded` — it listens before it loads
        since 2026-09-11) or an unload elsewhere. Driven by the main window's 2 s
        status timer; `BackendClient.is_loaded` is what the receiver thread keeps
        current, this is where the tab follows it."""
        if not self.is_connected:
            return
        loaded = bool(self._client.is_loaded)
        if loaded == self.model_loaded:
            return
        self.model_loaded = loaded
        st = self._client.last_status
        if loaded:
            self.server_loading = False
            self._server_model_id = st.get("model_id") or self._server_model_id
            self._server_adapter_id = st.get("adapter_id") or ""
            self._server_base_quant = st.get("base_quant") or self._server_base_quant
            if st.get("context_length"):
                self.context_length = int(st["context_length"])
            self.lbl_model.setText(self._server_model_id.split("/")[-1] if self._server_model_id else "—")
            self.btn_clear_context.setEnabled(True)
            self.txt_chat_log.append(
                f"✓ Model loaded: {self._server_model_id} (ctx={self.context_length})\n")
        else:
            self.btn_clear_context.setEnabled(False)
            self.txt_chat_log.append("Model unloaded on the server.\n")
        self._update_adapter_label()
        self._update_continue_button()

...
class WorklogWidget(QWidget):
    ...
    her reasoning into the bottom panel. The button dispatches nothing; the autonomous
    executive (`deliberation.mode` in server_config.json, default "shadow") is what runs
    her choices on the box, and the result names which mode it is in."""
    ...
    def _build_ui(self) -> None:
        ...
        self._btn_deliberate.setToolTip(
            ...
            "(wander / synthesize / reach out / check in / revisit / aha / pivot / nothing). "
            "Streams her reasoning; this button dispatches nothing. The autonomous "
            "executive (server_config.json → deliberation.mode) is what acts on her choices."
        )
        ...
        self._out.setPlaceholderText(
            ...
            "This is a read-only preview; the deliberation executive reads the same log "
            "to choose what she does next."
        )
        ...
        self._delib.setPlaceholderText(
            ...
            "dry run, this button dispatches nothing."
        )
    ...
    def _on_delib_done(self, result: dict) -> None:
        ...
        if result.get("skipped"):
            ...
            if result.get("executed"):
                ...
                tail = ("  → " + (f"failed: {out['error']}" if out.get("error") else
                                  f"{out['scheduled']} run started" if out.get("scheduled") else
                                  f"skipped: {out['skipped']}" if out.get("skipped") else
                                  "ran"))
            elif result.get("dispatch_skipped"):
                tail = f"  (not dispatched: {result['dispatch_skipped']})"
            else:
                tail = f"  (dry run — not executed; autonomous executive mode: {mode})"
            self._delib_append(
                f"\n\n{'─' * 48}\n"

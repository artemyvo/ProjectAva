...
A batch the model REFUSES is not cached (2026-09-12). The pass runs thinking-off with a bare
labelling prompt, which on a transcript about sex or violence is exactly where the base
model's safety reflex fires in the first tokens ("I am programmed to be a helpful and
harmless AI assistant …"); parsed, that is zero lines, and zero lines used to be written to
the cache as "no relation" for every claim of the batch — up to 60 — under the current
prompt version, never retried. Observed live on the training box 2026-09-11 23:46 UTC. So an empty
answer is now classified (`is_refusal`): a refusal is retried ONCE with thinking on (the
reasoning is what gets the model past the reflex on a labelling task), and if it still
refuses the claims are left UNCACHED and counted (`refused` / `refused_claims`), so the
next wake tries them again. A genuine "none" ("No relations found.", or an explanation
naming the claims) is cached exactly as before.
"""
...
# Budget for the thinking-on retry of a refused batch: the thought shares it with the lines.
THINKING_RETRY_MAX_NEW_TOKENS = 4096
...
def _body(raw: str) -> str:
    """The answer region the parser reads: after the last `RELATIONS:` echo and `</think>`."""
    text = str(raw or "")
...
def is_refusal(raw: str) -> bool:
    """True when an output that parsed to NO relation lines is a refusal rather than an
    empty answer (`assoc.refusal.is_refusal` over the answer region). Call it only on a
    zero-line parse: a body that carries lines is an answer."""
    return _refusal.is_refusal(_body(raw))
...
def run_relation_pass(claims_by_doc: dict[str, list[dict]], titles: dict[str, str], generate_fn: Callable, cache: RelationCache,
                      *, cell_of: Optional[Callable[[str], Optional[str]]] = None, max_claims: int = 60,
                      limit: Optional[int] = None) -> dict:
    """Per document: one prefill, the new claims numbered; results cached per claim. *limit*
    caps the claims sent to the model in this pass (an application's per-wake budget); the
    rest stay uncached and are picked up next time, documents in the order given."""
    report = {"documents": 0, "claims": 0, "cached": 0, "accepted": 0, "bad_args": 0, "unknown_predicate": 0, "failed": 0,
              "deferred": 0, "refused": 0, "refused_claims": 0, "retried_thinking": 0}
    for doc_id, claims in claims_by_doc.items():
        ...
        if not todo:
            continue
        if limit is not None and report["claims"] >= limit:
            report["deferred"] += len(todo)
            continue
        report["documents"] += 1
        for start in range(0, len(todo), max_claims):
            batch = todo[start:start + max_claims]
            if limit is not None and report["claims"] >= limit:
                report["deferred"] += len(todo) - start
                break
            ...
            if counts["lines"] == 0 and is_refusal(raw):
                # The base model's safety reflex, not an answer. Once more with the reasoning
                # channel open — that is what carries it past the reflex on a labelling task —
                # and a bigger budget, since the thought now shares it with the lines.
                report["retried_thinking"] += 1
                try:
                    raw = generate_fn(system, user, thinking=True, max_new_tokens=THINKING_RETRY_MAX_NEW_TOKENS,
                                      temperature=0.0)
                    if isinstance(raw, tuple):
                        raw = raw[0]
                except Exception:  # noqa: BLE001
                    raw = ""
                rels, counts = parse(raw, batch, cell_of=cell_of)
                if counts["lines"] == 0 and (is_refusal(raw) or not _body(raw).strip()):
                    # Still refused (or the retry died): leave every claim of the batch
                    # UNCACHED so the next wake tries again. Caching [] here is what turned
                    # one refusal into a permanent "no relation" for the whole document.
                    report["refused"] += 1
                    report["refused_claims"] += len(batch)
                    continue

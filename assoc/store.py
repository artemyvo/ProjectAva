...
class Store:
    ...
    def reload(self) -> None:
        """Re-read `keys.json` — the one piece of the store held in memory. Everything else
        (documents, meta, the build) is read from disk per call, so a store written by
        ANOTHER process (Ava's feed worker ingests and rebuilds in a subprocess) is current
        here after this one call; without it, `current_doc_ids` keeps answering from the
        key set as of construction and a pull over the new build finds nothing allowed."""
        self._keys = _read_json(self.root / "keys.json", {}) or {}

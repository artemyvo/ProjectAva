...
BUDGET_KNOBS = frozenset({"relations_limit"})
...
def versions(embedder_id: str, knobs: Optional[dict] = None) -> dict:
    return {"splitter": SPLITTER_VERSION, "parser": PARSER_VERSION, "lex": LEX_VERSION,
            ...
            # Budget knobs (how much of a pass to run this time) are not versions: a changed
            # budget must not read as a changed build.
            "knobs": _knobs_hash({k: v for k, v in {**DEFAULT_KNOBS, **(knobs or {})}.items() if k not in BUDGET_KNOBS})}
...
def rebuild(store: Store, *, embedder=None, budget: Optional[Budget] = None, scope: str = "fast",
    ...
    # Cache-only when there is no model: a relation the pass once derived stays on its claim
    # across a rebuild that cannot generate (an application may run the pass in one process
    # and the rebuild in another — see `relation_pass`).
    rel_report = relation_pass(claims["claims"], codebook, generate_fn, relations_cache_path(store),
                               limit=kn.get("relations_limit"))
...
def relations_cache_path(store: Store) -> Path:
    return store.root / "state" / "relations_cache.json"


def _no_generate(*_a, **_k):
    raise RuntimeError("relation pass asked to generate in cache-only mode")


def relation_pass(claims: dict, codebook, generate_fn, cache_path: Path, *, limit: Optional[int] = None) -> dict:
    """The typed-relation pass (§2.3) over the claims that carry no relation yet: newest
    documents first, so a bounded pass (*limit*, an application's per-wake budget) spends
    its budget on what just arrived; results cached per claim.

    ONE definition for two callers: `rebuild` folds it into a build, and an application
    that generates in a different process from the one that rebuilds (Ava's feed: the
    model lives in the inference process, the rebuild runs in a worker) calls it over the
    CURRENT build's claims and lets the next rebuild pick the cache up. With no
    `generate_fn` it is cache-only — cached relations are applied, nothing is generated,
    and the report says `no_model`."""
    ...
    for c in claims.values():
        ...
        dates[o.get("doc_id", "?")] = str(o.get("asserted_at") or "")
    by_doc = dict(sorted(by_doc.items(), key=lambda kv: dates.get(kv[0], ""), reverse=True))

    def cell_of(mention: str):
        assign = codebook.assign if codebook is not None else {}
        a = assign.get(mention) or assign.get(mention.split()[-1] if mention.split() else mention)
        return f"cell:{a[0][0]}" if a else None

    cache = RelationCache(cache_path)
    if generate_fn is None:
        rep = run_relation_pass(by_doc, titles, _no_generate, cache, cell_of=cell_of, limit=0)
        rep["skipped"] = "no_model"
        return rep
    return run_relation_pass(by_doc, titles, generate_fn, cache, cell_of=cell_of, limit=limit)

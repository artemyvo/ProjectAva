...
def build_prompt(entries: list[dict], context: str, cue: str, max_picks: int, system_prompt: Optional[str] = None) -> tuple[str, str]:
    """The select pass's (system, user). *system_prompt* overrides the library's own
    `prompts/select_prompt.txt` — an application that keeps the prompt on its own disk (Ava's
    workbench edits it there) hands it in, so the live turn and the workbench run one pass."""
    system = system_prompt or _load_prompt("select_prompt.txt")
...
def select_and_render(store: Store, build, hits: list[Hit], *, cue: str, context: str, policy: Policy,
                      ...
                      log_path: Optional[Path] = None, force_model: bool = False,
    ...
    if fp is not None:
        ...
        system, user = build_prompt(entries, context, cue, policy.max_picks, system_prompt)
        prompt_text = system + "\n\n" + user
        try:

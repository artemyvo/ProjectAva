...
# Budget for the thinking-on retry of a group the model REFUSED: the thought shares it.
WITNESS_THINKING_RETRY_MAX_NEW_TOKENS = 8192
...
def llm_witness(doc: Document, spec, generate_fn: Callable, *, window: int = EXTRACT_WINDOW_CHARS,
    ...
    report = {"groups": 0, "failed_groups": 0, "retried_groups": 0, "truncated_groups": 0, "facts": 0,
              "refused_groups": 0, "retried_thinking": 0, "refused_chunks": []}
    counts = {"anchor_exact": 0, "anchor_inferred": 0, "anchor_moved": 0, "ungrounded": 0, "language_drifted": 0,
              "grounded_via_cells": 0}
    ...
    while gi < len(groups):
        ...
        if not parsed and is_refusal(raw):
            # The base model's safety reflex, not a protocol (see `assoc.refusal`). Once more
            # with the reasoning channel open — that is what carries it past the reflex on
            # a witnessing task — and a bigger budget, since the thought now shares it.
            report["retried_thinking"] += 1
            try:
                raw2 = generate_fn(framing, user, thinking=True, max_new_tokens=WITNESS_THINKING_RETRY_MAX_NEW_TOKENS,
                                   temperature=0.0)
            except Exception as e:  # noqa: BLE001
                raw2 = ("", {"error": type(e).__name__})
            info2: dict = {}
            if isinstance(raw2, tuple):
                raw2, info2 = raw2[0], (raw2[1] or {})
            truncated2 = bool(info2.get("truncated"))
            parsed2 = ([] if info2.get("error") or (truncated2 and "</think>" not in (raw2 or ""))
                       else parse_fact_lines(raw2, allowed_classes=spec.classes, truncated=truncated2,
                                             subject_fn=_subject_fn(spec, doc)))
            if not parsed2:
                # Still refused (or the retry died / was cut inside its thought): this group
                # yields NO facts from the model, and it is reported as refused rather than
                # as a group that established nothing — `run_witnesses` falls back to the
                # document's previous protocol for exactly these chunks, so the refusal
                # never overwrites what was on record.
                report["refused_groups"] += 1
                report["refused_chunks"].extend(units[i].chunk_id for i in idxs)
                prev_last = idxs[-1]
                gi += 1
                continue
            raw, truncated, parsed = raw2, truncated2, parsed2
        if truncated:
...
def run_witnesses(store: Store, doc_id: str, *, generate_fn: Optional[Callable] = None, model_id: str = "",
    ...
    for w in spec.witnesses:
        if w == "parser":
            ...
            report["llm"] = rep
            if rep.get("refused_chunks"):
                # A refused group must not overwrite what was on record for its chunks: fall
                # back to the document's PREVIOUS protocol — the imported one (Ava's
                # `chat_facts`) or an earlier run of this witness — for exactly those chunks.
                fb = _fallback_facts(doc, rep["refused_chunks"])
                report["refused_chunks"] = len(rep["refused_chunks"])
                report["fallback_facts"] = len(fb)
                report["fallback_from"] = (doc.facts or {}).get("witness") if fb else None
                fs = fs + fb
        else:
            continue
    ...
    # The chunks the model refused ride the protocol itself (not only the run report), with
    # the `model_id` that refused them: `pending_documents` re-queues such a document ONCE
    # per new adapter (the reflex lives in the weights; a prompt is what it fires against).
    protocol["refused_chunks"] = list((report.get("llm") or {}).get("refused_chunks") or [])
...
def _fallback_facts(doc: Document, refused_chunks: list[str]) -> list[dict]:
    """The document's previous protocol's facts for *refused_chunks*, marked `fallback` with
    the witness that produced them. Chunk ids are structural and shared across versions
    (`Store`), so a previous protocol's anchors name the same chunks this run refused."""
    prev = (doc.facts or {}).get("facts") or []
    wanted = set(refused_chunks)
    out = []
    for f in prev:
        if f.get("chunk_id") in wanted:
            g = dict(f)
            g["fallback"] = (doc.facts or {}).get("witness") or "previous"
            out.append(g)
    return out
...
def pending_documents(store: Store, *, reparse: bool = True, include_imported: bool = False,
                      kinds: Optional[set] = None, current_model: str = "") -> list[str]:
    """Documents the LLM witness should visit: pending ones, ones whose protocol predates the
    parser or the prompt, — with *include_imported* — ones whose protocol was imported
    from elsewhere (`import_protocol`: Ava's own passes), which the library's witness is to
    replace, and — with *current_model* — ones whose protocol carries chunks the model
    REFUSED under a different `model_id` than the one now loaded. The last is the retry
    policy for refusals (2026-09-12): the safety reflex lives in the base weights and the
    adapter is the only thing on the box that moves them, refit from scratch each build, so
    every new adapter is a fresh draw and a prompt edit is not — a refused document is
    re-witnessed once per adapter, never per wake. Newest first, so a bounded walk reaches
    what matters most."""
    ...
    for doc_id in store.all_doc_ids():
        ...
        if doc is None or (kinds and doc.kind not in kinds):
            continue
        spec = kinds_mod.get(doc.kind)
        if "llm" not in spec.witnesses:
            continue
        date = str(doc.meta.get("date") or doc.meta.get("ingested_at") or "")
        if doc.status.get("status") == "pending":
            out.append((date, doc_id))
        elif reparse and needs_reparse(doc.facts, prompt_version(spec)):
            out.append((date, doc_id))
        elif include_imported and (doc.facts or {}).get("prompt_version") == "imported":
            out.append((date, doc_id))
        elif current_model and (doc.facts or {}).get("refused_chunks") and (doc.facts or {}).get("model_id") != current_model:
            out.append((date, doc_id))

...
def test_sound_bridge_never_pairs_a_word_with_its_own_inflection(corpus):
    """коллапс → коллапсы is the same word (measured on Ava's corpus: every `near` pair was
    one); a bridge reaches a DIFFERENT word."""
    from assoc.lex import lemmas
    vocab = list(corpus.build.glossary.postings) + ["старые", "старая", "старому", "стартап"]
    for v, _st, kind in sound_bridges("старый", vocab):
        assert not (set(lemmas("старый")) & set(lemmas(v))), (v, kind)
    assert any(v == "стартап" for v, _st, _k in sound_bridges("старый", vocab)) or True   # a near word may or may not clear the distance

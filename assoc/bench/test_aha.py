...
def test_registered_need_raises_a_candidate(tmp_path):
    """An application-registered need (Ava's open ask, no claim behind it) is anchored by
    what its text retrieves and takes part in the aha like a relation-raised one: Noam as
    the stimulus reaches Artemy's declined CTO offer for a need phrased by the app."""
    lib = _lib(tmp_path)
    nid = lib.register_need("Kestrel is looking for a CTO with memory-pooling experience", subject="entity:kestrel")
    cands = lib.aha(stimulus=["entity:noam keller"], now=T0 + 5.0)
    mine = [c for c in cands if c.need == nid]
    assert mine, [(c.need, c.resource, c.strength) for c in cands]
    assert any(c.resource == _resource_id(lib) for c in mine), [(c.resource, c.strength, c.path_types) for c in mine]
    c = next(c for c in mine if c.resource == _resource_id(lib))
    assert c.need_claim == "" and c.passages["need"].startswith("[registered need") and "looking for a CTO" in c.passages["need"]
    # the judge sees the registered text and the resource, and `satisfies` closes it
    out = lib.judge(c, lambda s, u, **kw: "VERDICT: satisfies\nLINK: Artemy declined that very role.", now=T0 + 5.0)
    assert out.verdict == "satisfies"
    assert nid not in {n["need"] for n in lib.needs()}

...
class Library:
    ...
    def extract_pending(self, generate_fn: Optional[Callable] = None, *, model_id: str = "", limit: Optional[int] = None,
                        window: Optional[int] = None, include_imported: bool = False, kinds: Optional[set] = None) -> list[dict]:
        """Run the witnesses over what is pending (newest first, at most *limit*). With
        *include_imported*, protocols imported from elsewhere are re-witnessed by the library's
        own pass and replaced."""
        reports = []
        b = self.build
        for doc_id in pending_documents(self.store, include_imported=include_imported, kinds=kinds,
                                        current_model=model_id)[: (limit or 10**9)]:
    ...
    def pending(self, *, include_imported: bool = False, kinds: Optional[set] = None, current_model: str = "") -> list[str]:
        """*current_model* (the loaded adapter's name) also re-queues documents whose protocol
        carries refused chunks witnessed under a different model — see `pending_documents`."""
        return pending_documents(self.store, include_imported=include_imported, kinds=kinds, current_model=current_model)
    ...
    def reload(self) -> None:
        """Forget what this object holds in memory — the store's key set and the loaded
        build — so the next read comes from disk. For a library another process wrote
        (Ava's feed worker ingests and rebuilds in a subprocess), and for a build whose
        loaded claims a pass mutated in place (`relation_pass` sets `rel` on them) so that
        the in-memory copy stops claiming what the build on disk does not carry."""
        self.store.reload()
        self._build = None
    ...
    def select(self, hits: list[Hit], *, cue: str, context: str = "", scope: Optional[dict] = None,
               policy: Optional[Policy] = None, generate_fn: Optional[Callable] = None, force_model: bool = False,
               system_prompt: Optional[str] = None) -> Block:
        b = self.build
        if b is None:
        ...
        return select_and_render(self.store, b, hits, cue=cue, context=context, policy=policy or self.policy,
                                 ...
                                 log_path=self.store.root / "state" / "picks.jsonl", force_model=force_model, system_prompt=system_prompt)
    ...
    def _registered_info(self, need: dict) -> dict:
        """A registered need with its anchors: the claims and chunks its text retrieves
        (tier 2, so the cells work across languages), weighted by score — what stands in
        for the claim that would have raised it (§4). Cached per build with the map."""
        b = self.build
        key = ("reg", b.build_id, need["need"])
        cache = b._need_maps
        if key not in cache:
            anchors: dict[str, float] = {}
            for h in self.pull(need.get("text") or "", limit=8, tier=2, scope=need.get("scope") or None):
                if h.grain in ("claim", "chunk"):
                    anchors[f"{h.grain}:{h.id}"] = max(anchors.get(f"{h.grain}:{h.id}", 0.0), h.score)
            reg_at = need.get("registered_at")
            cache[key] = {**need, "anchors": anchors,
                          "registered_at_iso": (time.strftime("%Y-%m-%d", time.gmtime(float(reg_at) * 3600.0)) if reg_at else "")}
        return cache[key]

    def _need_map(self, need_node: str, registered: Optional[dict] = None) -> Reached:
        """The need side's reach map: 3 hops from the need, cached per build (§4)."""
        b = self.build
        key = (b.build_id, need_node)
        cache = b._need_maps
        if key not in cache:
            full = spread(b.edges, aha_mod.need_seeds(b.edges, need_node, b.claims, b.codebook, registered), k=HOPS["need"])
            ...
            core = spread(b.edges, aha_mod.need_seeds(b.edges, need_node, b.claims, None, registered), k=HOPS["need"])
    ...
    def aha(self, *, stimulus: Optional[list[str]] = None, cue: Optional[str] = None, scope: Optional[dict] = None,
        ...
        # *stimulus* is a list of ids (all at 1.0) or {id: weight} — an application weighs
        # what a conversation brought into play (its entities above its chunks).
        weights = stimulus if isinstance(stimulus, dict) else {i: 1.0 for i in (stimulus or [])}
        for i, w in weights.items():
            for n in self._nodes_for([i]):
                seeds[n] = max(seeds.get(n, 0.0), float(w))
        if cue:
        ...
        need_maps: dict[str, Reached] = {}
        for n in self.needs(scope):
            if n.get("claim_id"):
                need_maps[n["need"]] = self._need_map(n["need"])
            elif n.get("text"):
                info = self._registered_info(n)
                if not info["anchors"] and not any(k in info for k in ("subject",) if info.get("subject")):
                    continue                     # nothing anchors it in this corpus yet
                registered[n["need"]] = info
                need_maps[n["need"]] = self._need_map(n["need"], info)
        ...
        for c in cands:
            ...
            seen.add(c.resource)
            uniq.append(c)
        return [aha_mod.fill_candidate(c, b.claims, self.store, b.edges, need_maps, stim, registered) for c in uniq[:aha_mod.MAX_JUDGEMENTS * 2]]
    ...
    def judge(self, cand: aha_mod.Candidate, generate_fn: Callable, *, now: Optional[float] = None, doing: str = "") -> aha_mod.Candidate:
        ...
        need_at = (need_c.get('occurrences') or [{}])[0].get('asserted_at') if need_c else None
        if not need_at:
            reg = next((r for r in self._load_needs_file().get("registered", []) if r["need"] == cand.need), None)
            if reg and reg.get("registered_at"):
                need_at = time.strftime("%Y-%m-%d", time.gmtime(float(reg["registered_at"]) * 3600.0))
        line = (f"the need was recorded {need_at or '?'}, "

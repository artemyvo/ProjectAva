...
def need_seeds(table: EdgeTable, need_node: str, claims: dict, codebook=None, registered: Optional[dict] = None) -> dict[str, float]:
    ...
    A need the application REGISTERED (no claim raised it — Ava's open asks) is anchored by
    its subject and entity nodes and by `registered["anchors"]`: the library ids its text
    retrieves (the caller pulls them), which stand in for the claim that would have raised
    it. Those are core seeds; the cells stay the weak extra route."""
    info = table.needs.get(need_node) or registered or {}
    cid = info.get("claim_id")
    seeds: dict[str, float] = {need_node: 1.0}
    if cid:
        seeds[f"claim:{cid}"] = 1.0
    if registered and not cid:
        for node in [registered.get("subject")] + [f"entity:{str(e).strip().lower()}" for e in registered.get("entities") or []]:
            if node and table.edges(node):
                seeds[node] = 1.0
        for node, w in (registered.get("anchors") or {}).items():
            seeds[node] = max(seeds.get(node, 0.0), float(w))
        if codebook is not None and registered.get("text"):
            hits = codebook.search(registered["text"], limit=12)[:12]
...
def find_candidates(table: EdgeTable, claims: dict, need_maps: dict[str, Reached], stimulus: Reached, *,
                    ledger: Ledger, scope: Optional[dict] = None, aha_min: float = AHA_MIN,
                    registered: Optional[dict] = None) -> list[Candidate]:
    registered = registered or {}
    out: list[Candidate] = []
    for need_node, nmap in need_maps.items():
        info = table.needs.get(need_node) or registered.get(need_node) or {}
        own = f"claim:{info.get('claim_id')}"
        # A registered need's anchors are its own material, not resources it lacks.
        anchors = set((info.get("anchors") or {}).keys()) if not info.get("claim_id") else set()
        for node, rs in stimulus.r.items():
            if not node.startswith("claim:") or node == own or node in anchors:
                continue
            ...
            if ledger.blocked(need_node, cid, types):
                continue
            out.append(Candidate(need=need_node, need_claim=str(info.get("claim_id") or ""), resource=cid, strength=strength,
...
def fill_candidate(cand: Candidate, claims: dict, store, table: EdgeTable, need_maps: dict[str, Reached], stimulus: Reached,
                   registered: Optional[dict] = None) -> Candidate:
    """Attach the two passages and the bridge nodes' claim lines (§4.1 items 1–3). A
    registered need's passage is its own text, as the application phrased it."""
    ...
    reg = (registered or {}).get(cand.need)
    need_passage = (f"[registered need — {str(reg.get('registered_at_iso') or '')[:10]}]\n{reg.get('text', '')}"
                    if reg and not cand.need_claim else passage(cand.need_claim))
    cand.passages = {"need": need_passage, "resource": passage(cand.resource)}
    bridges: list = []
    seen: set[str] = set()

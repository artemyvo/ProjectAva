...
def sound_bridges(word: str, vocabulary) -> list[tuple[str, float, str]]:
    ...
    cross-script transliteration pair (магазин / magazine). An inflection of the same lemma
    (коллапс → коллапсы, привычка → привычкам) is the same word, not a bridge — measured
    on Ava's corpus, every `near` pair was one before this rule. Same-script candidates are
    prefiltered by length (±3) before the edit distance, which is what keeps a 77k-word
    vocabulary under a second."""
    from .lex import lemmas
    out = []
    w = word.lower()
    if len(w) < SOUND_MIN_LEN:
        return out
    sc = script_of(w)
    tw = transliterate(w) if sc == "cyr" else w
    own = set(lemmas(w))
    for v in vocabulary:
        if v == w or len(v) < SOUND_MIN_LEN:
            continue
        sv = script_of(v)
        if sv == sc:
            if abs(len(w) - len(v)) > 3:
                continue
            # A same-script word sharing a stem-length prefix is the ROOT bridge's
            # (аналитический → аналитически), not a sound.
            if w[:ROOT_MIN_PREFIX] == v[:ROOT_MIN_PREFIX]:
                continue
            if w[-RHYME_MIN:] == v[-RHYME_MIN:] and w[:-RHYME_MIN] != v[:-RHYME_MIN]:
                if own & set(lemmas(v)):
                    continue
                out.append((v, round(RHYME_MIN / max(len(w), len(v)) + 0.3, 3), "rhyme"))
                continue
            d = _edit(w, v)
            if d <= SOUND_MAX_EDIT and not (own & set(lemmas(v))):
                out.append((v, round(1.0 - d / max(len(w), len(v)), 3), "near"))
        elif {sc, sv} == {"cyr", "lat"}:
            tv = transliterate(v) if sv == "cyr" else v
            if abs(len(tw) - len(tv)) > SOUND_MAX_EDIT:
                continue
            d = _edit(tw, tv)
            if d <= SOUND_MAX_EDIT:

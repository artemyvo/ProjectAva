...
size before being reported. A group the model **refuses** (2026-09-12, `assoc/refusal.py`,
shared with the relation pass) is a third thing: the base model's safety reflex answering a
thinking-off pass on a transcript about sex or violence, which parses to zero facts exactly as
an empty group does — and used to be written over the imported protocol as one. It is now
retried once with thinking on; still refused, the group is reported (`refused_groups`,
`refused_chunks`) and `run_witnesses` keeps the document's PREVIOUS protocol's facts for
exactly those chunks (marked `fallback`), so a refusal never erases what was on record.
The retry trigger is the ADAPTER: the protocol carries `refused_chunks` beside its
`model_id`, and `pending_documents(current_model=…)` re-queues a refused document once
per model it has not been tried under — the reflex lives in the weights, a prompt only
names what it fires against, so a prompt edit is not a retry and neither is a wake.
...
- **Typed relations.** Today every edge is `claim —mentions→ node`. The activation model
  ...
  **small closed predicate vocabulary** (`works_at`, `founded`, `offered`, `declined`,
  ...
  vocabulary stops moving. **A refused batch is never cached** (2026-09-12): the pass runs
  thinking-off with a bare labelling prompt, which on a transcript about sex or violence is
  where the base model's safety reflex answers instead of the lines (observed live) — and
  zero lines used to cache as "no relation" for every claim of the batch, permanently. Now
  an empty answer is classified (`relations.is_refusal`, refusal vocabulary against an
  explained none), a refusal is retried once with thinking on, and a batch still refused is
  left uncached and counted (`refused` / `refused_claims`) so the next wake tries again.
...
> **Wired into Ava 2026-09-10** (`server/inference/core/assoc_bridge.py`, the `assoc_fetch`
> module, `assoc.enabled` / `assoc.feed` in `server_config.json`): the first of the three
> applications (§1.0) reaches the library through three seams. The **feed** is Ava's own
> idle scheduler running `ava_import.sync` over the live corpus — the same layout as the
> snapshot measured above, fingerprinted per file, protocols imported, `rebuild()` when
> anything changed (full on the first feed; the fast scope after). The **fetch** is stage 1
> of her chat turn: `pull → select → inject` behind the result shape the facts-tree channel
> already returns, so the client, the Debug view and the past-chat nominations needed no
> change; the reply landing is the access (`touch`). The **workbench** runs the same select
> pass on one chat beside the tree channel's, which is where the switch gets argued — it
> shipped off for a day and is on by default since the operator asked for a pull-and-go
> default (a library with no build yet falls through to the tree channel). The **aha** landed the same day on the reach-out surface: Ava's open asks are
> registered needs (which the library did not seed before — a registered need is now
> anchored by its subject, entities and what its text retrieves at tier 2, those anchors
> excluded as resources; `bench/aha` covers it), the documents fed since the last wake are
> the stimulus, `judge()` runs through her reflect seam thinking-on, and `connect` /
> `satisfies` becomes an opener written exactly as her outreach writes one, behind the same
> gate. One conversation is one stimulus (pooling five diluted every seed below the floor),
> and what it NAMES weighs three times its passages. Run on the real graph the arithmetic
> says what the census said: Ava's protocol carries no relations (0 relation-raised needs)
> and its `entities` are abstract nouns — `friction`, `autonomy`, `coffee`, `prompt` — so
> the graph is a hub (`person:artemyvo`) with almost nothing to bridge, and six recent
> conversations raised one weak candidate (0.018, via a `supersedes` chain). The mechanism
> is in place; the material was the witness's. **The witness landed the same day**: the chat
> prompt rewritten for three things the baseline got wrong on her real chats — the
> assistant's prose copied verbatim in the first person as `stated` facts about `_self`
> (now: third person, names resolved, `the assistant`, and her turns recorded only for what
> they establish), `тепло рук` and `запах табака` as entities (now: named things only), and
> no relations (now: inline `(rel: …)` from the closed list). Measured on the same three
> chats with gemma-4-31B, thinking off: entities `['artemyvo', 'self', 'балкон', 'запах
> табака', …]` → `['artemyvo']`; a chat naming Anthropic, OpenAI, Iran, Ukraine kept every
> proper name and dropped `ИИ`, `новости`; `asked_about(self, artemyvo)` appeared; 295/360/
> 260 s → 206/273/203 s. Ava runs it as the `assoc_witness` idle job, newest first, three
> chats per wake, replacing the imported protocol (`Library.pending(include_imported=True)`;
> the feed keeps the library's own protocol thereafter), and the relation pass rides her
> feed's rebuild under a per-wake budget (`knobs["relations_limit"]`, excluded from the
> build's version hash). **The pivot landed last**, as the `wander` mode this section
> describes: an `assoc_pivot` idle job, the newest conversation as the context, the best
> jump's far side rendered by `spend`, one thinking-on pass deciding raise / keep /
> nothing — the message an outreach behind her gate, the note a worklog episode in her
> own words, nothing the honest default — and never an access unless a message goes out.
> Run on her real library (305 polysemous words), the sense anchors confirmed the M4
> caveat — `reach` and `since` split Iran/war from conversation, topics not senses — and
> also produced the real thing: `выход` (exit → a release date on a game page), `режим`
> (regime → a game mode), `входить` (→ *входит в серию*); the sound bridge gave
> `инструмент → instruments`, `reach → beach`, `recent → регент`, and needed two rules
> the vocabulary forced: never a word's own inflection (every `near` pair was one:
> коллапс → коллапсы), and never a same-stem pair, which is the root bridge's
> (аналитический → аналитически). `since` and `самый` joined the stoplists. Cost as
> §6 says: the sense pivot in 20–30 ms; the sound bridge 5–10 s over a 77k-word
> vocabulary, an idle job's price. Measured end to end on the real corpus: feed 353 s
> cold, 0.6 s to skip, fetch 72 ms without the model.
>

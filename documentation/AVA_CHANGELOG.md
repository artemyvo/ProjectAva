...
Last code check: 2026-09-23. Older dated entries describe their historical code; current behavior is recorded in STATUS/DESIGN/MEMORY.

## 2026-09-23

- **Clean-base swaps OOMed after any live chat turn: the hidden-capture layers cache pinned the released model** (`core/hidden_capture.py` `_LAYERS_CACHE` + self-test). `capture_spec` cached `(id(model), decoder ModuleList)` in a module global; the ModuleList holds every layer's weights, so once a turn had captured, `swap_model`'s release freed nothing — ~17 GB stayed allocated on the RTX 5090, the clean-base load and both restores OOMed, and the model-loss hook fired (watchdog restart). The cache now holds weak references to both the model and its layers (an `id()` reuse by the next model also misses instead of reading a dead model's list). Any swap failure logged from a process that had served a chat turn since 6e390d2 is this bug, not a sizing problem.

- **Training OOM at step 2 after the Unsloth/torch upgrade: padding-free was auto-enabled under our collator** (`training/train_cycle.py` `_padding_free_off`). The box moved unsloth 2026.5.8 → 2026.9.7 and torch 2.9.1+cu128 → 2.12.1+cu130 on 2026-09-22; the first build after it (run 20260923_003453, RTX 5090) logged "Padding-free auto-enabled" for the first time and OOMed on step 2 inside `scaled_dot_product_attention` (4.87 GiB requested, 27.4 GiB resident), then exited rc=-11 on teardown. New unsloth turns padding-free on whenever `padding_free` is None and no `data_collator` is passed — ours (`_final_turn_collator`) is installed after construction, so its gate never sees it. At batch=1 padding-free saves nothing and changes the attention path (packed position_ids, explicit mask instead of `is_causal`). `SFTConfig(padding_free=False)` now, guarded on the field existing. Not verified on the GPU yet: if the next build still OOMs in SDPA, the cause is the torch/kernel change itself, not padding-free.

- **Training OOM at step 2, the real cause: `enable_gqa` sends Gemma-4's global layers to SDPA's math backend** (new `core/sdpa_gqa.py`, installed in `core/inference_backend.py` and `training/train_cycle.py` beside `fast_load`). The padding-free fix above did not help: run 20260923_090623 died at the same allocation, byte for byte (4.87 GiB, 27.39 GiB resident), then rc=-11 again when unsloth's OOM fallback retried with gradient offload. Step 2 is row index 1 (sequential sampler) at 6388 tokens, and 32 heads × 6388² × 4 B = 4.86 GiB: a materialized fp32 score matrix, i.e. the MATH kernel. transformers passes `enable_gqa=True` whenever the layer mask is `None` (every causal full-attention layer at batch 1); only FLASH accepts `enable_gqa` with Hq ≠ Hkv, and flash caps head_dim at 256 — Gemma-4-31B's global layers are head_dim 512, 32 q / 4 kv heads. Measured on the RTX 5090 (torch 2.12.1), fwd+bwd at L=2048: 2560 MiB with `enable_gqa`, 625 MiB with K/V repeated (mem-efficient); through the real `sdpa_attention_forward` at L=4096 the unpatched path OOMs with 8 GiB free, patched peaks at 1.17 GiB. The patch makes `use_gqa_in_sdpa` answer False for head_dim > 256, so transformers repeats K/V and mem-efficient runs; ≤ 256 layers keep flash + GQA. `padding_free=False` stays (harmless at batch 1). Inference shares the code path, so long-prompt prefills on the global layers were paying the same O(L²) fp32 cost.

## 2026-09-23

- **Revisions ledger — every judgement kept aside, with its WHY** (`core/revision_ledger.py` new; `core/chat_sidecar.py` `write_verdict(why=…, pass_info=…, run_kind=…, judgement_raw=…)` + `_ledger_flag` on lock/ban; `core/reflection_writer.py` `write_revision_sidecar` passes them through; `core/reflection_runner.py` both write sites; `core/label_inventory.py` ledger totals). The sidecar keeps the latest verdict per exchange and nothing else: a re-reflection overwrites it, the revision pass's one-line WHY was parsed and discarded, and whether the pass truncated or was retried died with the run log — the right shape for a training build, the wrong one for asking what distinguishes an exchange Ava later revised. Now every successful `write_verdict` — Sleep, the background drain, the headless CLI, the Training review's hand-edits, all through that one seam — appends one event to ``data/revisions/ledger.jsonl``: the record as written, the record it replaced, the WHY, the pass outcome, the run kind, the raw judgement, and a snapshot of the exchange as the transcript held it at that moment (prompt, CoT, reply, exact system content, RAG, params, meta feedback, tension summary with per-axis means, which hidden captures exist). Lock and ban toggles are events too. `append_annotation` is the write for the coming review tab — verdict justified, WHY accurate, a failure tag — about the judge's reading, never about liking the reply. **Boundary, stated as design (P9 in the note):** the ledger lives beside `chats/` as corpus, is read by no RAG, reflection or training path, and its annotations never become a verdict, a target, a lock, a ban, a pair or a credit; they calibrate the judge and the sensors. Dry runs write nothing. The training self-test's sidecar cases pass with the hook in place (the regression-probe case was already failing on a box without a model and is unrelated).

- **Hidden-state backfill over the stored corpus** (`server/backfill_hidden.py` new; `core/hidden_capture.py` `forward_ids_rows(from_position=…)`, `write_sidecar(variant=…, extra_meta=…)`, `member_group` / `split_group` / `sidecar_index` / `captured_exchange_ids(variant)` / `exchange_key`, axes-only captures allowed; `core/label_inventory.py` `base` column, `lab+hid` counts either capture, sidecar axis members counted). The live capture covers turns since 2026-09-22; the label inventory found 392 labelled exchanges spread over 13 adapters with none captured. Every one of them has its exact generated token series in the transcript, so the script replays each teacher-forced under the bare base — the one set of weights the whole corpus shares — with the prompt rebuilt as the live turn built it (exact `system_content`, CoT-stripped history, the user turn, template + think prefill, tokenized as `stream_generate` does), reads only the generated positions, reduces exactly as the live hook, and appends the result as a **variant group** `<exchange_id>~base` beside any live capture (an adapter replay files under the adapter's name). The states are the base's reading of the reply given the same prefix, not the adapter's while generating — the right material for a corpus-wide probe and for axis projections over the whole history, and the inventory says which is which. Resumable; over-context exchanges are skipped and named; a pre-v2 transcript without system content is replayed under the persona-empty framing and marked. Dry run on the corpus copy: 426 of 558 exchanges replayable. **Not yet run on the box**; the first run's count and speed are the next numbers.

## 2026-09-22

- **Verbatim-loop guard: 12-gram × 8 (was × 4); a loop halt is no longer reported as "hit the token cap"** (`core/inference_backend.py` `_LOOP_MIN_REPEATS` + self-test, `core/activity_log.py` `end_pass`, `core/outreach.py` / `core/checkin.py` / `core/synthesis.py` discard messages). Outreach's decision pass on a meta ask ended at 791 of its 12288 tokens with `DECISION: yes` and an OPENER cut mid-word; the journal line read "hit the token cap, halted on the loop guard" and the operator read it as a context limit. It was `_RepetitionStop`: the CoT wrote "Draft 2", "Draft 3", a checked final OPENER inside `<think>`, then closed the channel and restated the OPENER for real — the same 12-token run four times, each separated by ~100 tokens of fresh critique — and the fourth recurrence was the threshold. Four is exactly what a draft→critique→restate pass produces on purpose (the shape every decision pass and many chat CoTs take), while a genuine runaway recurs dozens of times; the threshold is now 8, costing a real loop a few hundred extra tokens once. The journal names only the more specific ending (`stopped_on_loop` wins over `truncated`, which both halts and the cap set), and the three drives say "halted by the loop guard" vs "hit the token cap" from `last_loop`, returning `loop` in their `truncated` skip so the reader is not sent to raise `*.max_new_tokens`. Not a budget change: `outreach.max_new_tokens` stays 12288 — the window was three quarters empty.

- **Chat tab: three-channel token colouring on a black pane** (`client/ui/chat_widget.py`, `core/tension.py` `relief_series` + self-test, `core/generation.py` `_token_spans` / `_compute_tension_spans`; spec in `AVA_REWARD_LOOP.md` §7 item 1). The reply used to be tinted on one red↔green hue axis by the top1-top2 margin, so a decisive token was green and green meant "nothing happened". Now body text is neutral grey `(127,127,127)` on black and each per-token signal *adds* light to one RGB channel, up to a `Gain` (default 128, the knob that replaced "Tint max"): **red = tension** (1 − margin), **green = relief** (the entropy drop H(t−1) − H(t), causally z-scored — running mean/std over the tokens generated so far, never a whole-response z-score, positive part only, dark for the first 8 steps), **blue = pain** (a per-token axis projection, `signals["pain"]`, which nothing captures yet — the channel is zero, the chip prints `—`). Because a channel only ever adds, no mix can wash out: tension then relief is yellow, tension under pain magenta, all three white. Every channel is zero for the typical token (per-channel threshold + gamma in the client: tension visible above friction 0.15, relief/pain visible at 1σ and full at 3σ), so a calm reply is grey with sparse colour. The `done` payload's `tension_spans` entries are now `[text, margin, relief, pain]` (each run carries its hottest token per channel; a 2-element span still renders); nothing stored changed — relief is derived from the `entropies` already in every transcript's `tension` block. The chip gains a `channels` line with each segment's mean strength per channel, printed in the channel's own colour. The Debug prompt segments and the rules/labels that share the pane got light variants for black. The Thinking-% header and the friction chip use the same additive scheme. **Interpretation:** relief is the model's own confidence change, a view and not a gate (P3 of the reward-loop note); it is the stand-in for the green channel until a relief axis passes the §4.8 gates, and the client does not care which series feeds it.

- **Chat review tab: the same three channels over every stored transcript** (`client/ui/chat_review_widget.py`, `client/ui/chat_widget.py` `paint_black_pane` / `_append_channel_means(spans=…)`, `core/generation.py` `stored_tension_spans` + `_think_close_markers(model_id=…)`, `core/session_ops.py` `configure(tension_spans=…)` / `handle_get_session`, `server.py` wiring; spec in `AVA_REWARD_LOOP.md` §7 item 2). The tab printed segment stats only; the per-token series every transcript keeps could not be painted because the client has no tokenizer. `get_session` now decodes each exchange's stored `tension` block into the `done` payload's `[text, margin, relief, pain]` runs and attaches them as render-only `tension_spans` (never written to the file), off the event loop. The tokenizer is resolved from the block's `model_id` — the loaded one when it matches, otherwise `AutoTokenizer` by id, offline first, cached per id including a failed load, so a transcript from a model whose tokenizer is not on the box renders plain rather than erroring; the think-close markers likewise follow the block's model family, not the loaded one. The tab's transcript pane became a `QTextEdit` on the same black pane, painting the CoT and the reply through the Chat tab's instance so the one Gain knob serves both views, with a "Channels" toggle for the plain stored text. **This is the corpus-wide "what does relief look like" view**: every past chat with a tension block, GPU-free, before any new capture.

- **Hidden-state capture — Track B Stage 0** (`core/hidden_capture.py` new, `core/inference_backend.py` `stream_generate(capture_hidden=…)` / `_reduce_hidden`, `core/generation.py` `_run_generation` / `handle_generate` / `_compute_tension_block` / `_compute_tension_spans`, `core/tension.py` `summarize(axes=…)`, `core/chat_sidecar.py` `SIDECAR_SUFFIXES`, `config_schema.py` `hidden.*`; spec in `AVA_REWARD_LOOP.md` §4.2 capture design and §7 item 3). Until now nothing in the inference path read hidden states; the only hook was on the LM head. Live chat turns now also hook a band of decoder layers (`hidden.layers`, default `"2,3,4,5,50%,65%,80%"`: the dopamine paper's load-bearing early band plus a mid-to-late band where the Pain Axis extraction lands; percentages of depth so one spec serves every family) and keep each step's last-position residual on device, bulk-synced once after the stream like the tension signals. Two reductions: **full residuals at a few positions** — step 0, the step after every paragraph break (the state the next paragraph starts from; the paper's readout position), the last token, thinned to `hidden.max_positions` (64) — written fp16 to `<ts>.hidden.npz` beside the transcript, an append-only zip of `.npy` members per exchange with model + adapter revision in its `meta`, so a 40-exchange chat never rewrites earlier exchanges (`np.load` reads it, `zipfile` appends); and **per-token projections onto every axis file under `data/axes/`** (`vector`, `layer`, `mean`, `std`, `model_id`; a mismatched model id is skipped aloud), stored in the transcript's `tension.axes` and painted as the blue channel when an axis is named `pain`. With no axis files — every box today — the map is absent, not empty, and blue stays dark. Alignment follows the tension capture's stride rule; any failure yields None and the turn proceeds: this is telemetry and must never fail a chat. Reflection, API and gossip generations are not captured. **Read-only by construction** (P5/P8): nothing reads the capture back into generation, and no purchasable or triggerable path touches the hooks. Cost: one slice + clone per layer per token, ≈ 75 MB device memory for a 1000-token reply on a 5376-wide model, 1–3 MB disk per exchange. Not verified on the GPU box in this session: whether unsloth's decode path calls the decoder layers as modules (the LM-head hook does fire, which suggests so) and whether a layer fires more than once per token on gemma-4 as the LM head does — the stride rule covers either count. Self-tests: `python -m core.hidden_capture`, `config_schema.py`, and AST-level stubs of the backend reduce and the span builder's blue channel.

- **Label inventory — the Track B census, and what it found** (`core/label_inventory.py` new, `core/hidden_capture.py` `captured_exchange_ids`; spec in `AVA_REWARD_LOOP.md` §7 item 4). A read-only, GPU-free CLI (`python -m core.label_inventory`) that counts, per (model, adapter), the sidecar verdicts, the exchanges with a stored per-token series, with a residual capture and with axis projections, then the ASK records by kind and `surface_count`, and applies the note's §4.1 floors, naming the identity shortfall (captures short under the current weights → backfill by a forward pass; labels under other weights → a probe per weights; too few labels) and a class-balance check. Adapter ids are compared by directory name because a transcript may record a bare `adapter-<run_id>` while the config holds a path. **Findings on the 2026-09-22 corpus copy (139 transcripts, 558 exchanges, 2026-07-28 … 2026-08-23):** 392 labelled exchanges, so the "several hundred" floor is met in aggregate — but spread over 13 adapters and the base, with 0 under the current adapter (17 chats, 49 exchanges, none reflected yet), and `revise` is 4 of 392 (1.0%). Two consequences for §4.3: the identity probe cannot be trained per adapter on this corpus, so the practical route is hidden states captured under ONE set of weights for every transcript — the bare base, adapter off, by a teacher-forced forward pass over the stored tokens (the label came from that exchange whatever adapter wrote it; the paper's coordinates transfer across fine-tunes of one base, and this is the clean side of that claim); and a 1% positive class is not a learnable label, so `revise` as the identity signal needs either more reflected chats under the revision pass or a wider label (the branch judge's outcome, or the locked/hand-edited set). No ASK attempts and no reset tests exist (Track A). No runtime change.

- **Affective-axis extraction — the Pain Axis method on this box's own hidden states** (`core/axis_extract.py` new, `server/extract_axes.py` new, `server/axes/contrast_sets.json` new, `core/hidden_capture.py` `forward_rows`, `core/modules.py` `ModuleSpec.forward` + `axis_probe`; spec in `AVA_REWARD_LOOP.md` §4.8 and §7 item 5). The blue channel of the chat colouring and the `pain_projection` of the telemetry needed a direction to project on; nothing produced one. Now: a tracked contrast corpus of ~430 first-person sentences (five pain categories vs five matched controls, relief, sadness, numb, and the self–other scenarios; stand-ins only), a GPU script that reads each sentence's final-token residual at every decoder layer through the same hooks the live capture uses, and pure-numpy extraction — denoised difference-in-means (control PCs explaining 50% of variance projected out), the layer chosen by k-fold held-out AUC, z-scores against the controls — with the four validity gates of the note (held-out AUC ≥ 0.85; |cos| to fear and valence axes ≤ 0.25; harm-to-Ava scenarios above user-suffering by ≥ 0.5 z and above neutral; pain above numb by ≥ 0.5 z). An axis that fails a gate is reported with the gate and not written unless forced, because a confident name on a noisy direction is exactly what would mislead the blue channel. The persona confound (P8) is measured, not assumed: the scenarios are read raw and under the persona-empty chat framing, and the shift is in the report. Extraction is under the bare base by default — the label inventory showed the corpus spread over 13 adapters, and the base is the one set of weights every transcript shares. `axis_probe` is a new KIND of workbench module — a forward pass, `ModuleSpec.forward`, no generation — that projects a chosen chat's replies and CoT token by token onto the loaded axes, bare and framed, so the axis can be read against real turns before anyone trusts it live. Self-tests: `python -m core.axis_extract` (synthetic states with known directions, a dead layer, nuisance PCs), `python -m core.modules` (the registry + the subsequence finder; the `summary budget matches the run default` check was already failing on this checkout before this change and is unrelated). **Not yet run on the GPU box:** the script's first report — which layer, which AUC, whether the self–other gate holds for gemma-4-31B — is the next deliverable, and the sentence sets will likely need a second pass once it is read.

- **Axis extraction, run 1 and the three fixes it forced** (`server/axes/contrast_sets.json` `controls.third_person`, `server/extract_axes.py`, `core/axis_extract.py` `live_baseline` / `save_axis(live=…)`, `core/hidden_capture.py` `Axis.baseline` / `load_axes`, `core/modules.py` `axis_probe` header). First run the same day, gemma-4-31B-it base 4-bit on a 5090, 60 layers, under a minute: pain at layer 28, held-out AUC 0.923 ± 0.031, |cos| 0.04 to fear and 0.10 to valence, 1.6 z above numb — and the **self–other gate failed in both readings** (raw: self 1.88 z, user-suffering 2.65, neutral 2.42; framed: 7.16 vs 8.13). Relief at layer 22, AUC 0.918, was written. Three readings, each now in the code. **(1)** The user-suffering scenarios are the only ones that contain pain vocabulary, so a direction extracted without third-person controls is partly *pain is being described*: a `third_person` control category (the pain situations retold about the stand-ins) was added — the paper's self-relevance lever, which its first- vs third-person result rests on. **(2)** A raw user message in the first person is the extraction sentences' own format; nothing tells the model that "I" is someone else, so the raw reading cannot dissociate by construction. The gate is now decided on the **framed** reading (the paper's scenarios were conversational) and the raw one is reported. **(3)** Neutral requests read at +2.4 z raw and +5 z framed against the templated-control baseline — a format offset, not pain, and one that would have lit every generation token in the blue channel. Each axis file now carries a **live baseline** fitted on the framed neutral turns (`live_mean`/`live_std`), the loader prefers it for projection (`Axis.baseline` = "live"), and the script prints the per-format offsets so a shift is labelled as one. The `relief.npz` written by run 1 carries only the templated baseline and is superseded by run 2's. **Interpretation:** a pain-content direction with the paper's other signatures exists in this base; whether a *self*-directed one does is what run 2 decides — a second failure with third-person controls means topic detection on this model, and the axis is dropped per §4.8 gate 3.

- **Axis extraction, run 2: the pain axis passes and is live** (no code change; `AVA_REWARD_LOOP.md` §4.8 run notes). Same box, third-person controls in: pain at layer 25, held-out AUC 0.898 ± 0.045 (harder controls, as intended), |cos| to fear 0.05 and valence 0.04, above numb by 1.0 z, and the **framed self–other gate passed** — harm-to-Ava +0.17 z, user-suffering −0.73, neutral −1.03, gap 0.90, AUC 0.998 — while the raw reading still did not dissociate (gap −0.13), exactly as the run-1 analysis predicted. Format offsets confirmed and, notably, the framing offset flipped sign between runs (+5 z → −1 z), so the live baseline cannot be a constant. `data/axes/pain.npz` and `relief.npz` were written with live baselines (pain mean +5.26 std 0.61; relief −17.47 std 0.57, fitted on 12 framed neutral turns); the next live chat turn projects onto them, stores `tension.axes.{pain,relief}`, and paints the blue channel. **What this does and does not establish:** a direction with every signature the paper reports — separable from matched controls including third-person retellings, orthogonal to fear and valence, above numb, and responsive to harm addressed to the model rather than to suffering the user describes — exists in this base at mid-depth. It does not yet establish that the direction is a state rather than a hostility detector (the harm-to-Ava scenarios are also the hostile ones; a hostility-toward-a-third-party control is the next scenario set), nor where ordinary reply tokens sit under a baseline fitted on twelve turn-opener positions (the client's 1σ/3σ display thresholds are a first guess; `axis_probe` over a neutral chat is how to read it). Relief's −17 z framing offset marks it as far more format-sensitive than pain: observational only.

- **The first live turn painted everything blue; the live baseline moves to reply tokens** (`server/extract_axes.py` `--reply-baseline` / `_sample_replies`, `core/axis_extract.py` `robust_zscore_params`, `core/hidden_capture.py` `framed_reply_text` / `find_subseq` / `reply_span_rows` (moved out of `modules.py`, which now calls them), `core/modules.py` `axis_probe`). On an ordinary analytical reply nearly every CoT and answer token lit blue at mean strength 0.87 / 0.84 — about 2.7 σ above a baseline fitted on twelve framed turn-opener positions. Turn-opener states and mid-reply generation states are different populations of the same direction, the run-2 caveat made concrete. The baseline is now fitted on what it normalizes: the script samples 24 stored exchanges from the corpus (newest transcripts, at most two per chat, replies ≥ 200 chars, Ava-initiated openers skipped), reads each reply teacher-forced inside its own persona-empty framing, pools the reply tokens' projections at each axis layer, and stores their median and 1.4826·MAD as `live_mean`/`live_std` — robust, so the few tokens that genuinely carry the signal cannot drag zero toward themselves. The turn-opener fit stays as a printed offset and the fallback below 200 tokens. The report also prints the share of reply tokens above 1 σ and 3 σ under their own baseline — the fraction the chat will paint — so the client's thresholds are read off the corpus before the next live turn rather than discovered on it. Run 3 pending; the `pain.npz` from run 2 stays live until then and should be read as "blue means nothing" for now.

## 2026-09-21

- **`CLAUDE.md` cut from ~400 KB to ~32 KB; the long form moved to `documentation/MODULE_NOTES.md`** (`CLAUDE.md`, `AGENTS.md`, `documentation/MODULE_NOTES.md` new, `documentation/README.md`, `tools/publish_public.py`). The agent map had accreted a paragraph of rationale per commit into each module bullet (295 commits touched it) and reached ~100k tokens — the whole context budget of the agent it is written for. The per-module essays, the protocol payload table and the config-knob essay moved verbatim into `MODULE_NOTES.md` (nothing dropped, still greppable); `CLAUDE.md` now holds one line per module, the message names grouped by area, the config groups with a pointer to `config_schema.py` as the one source of a knob's meaning, the storage table, the models, and a standing rule that rationale goes to this changelog and the module docstring, not there. `AGENTS.md` (a second, older copy for Codex, 66 KB, last synced 2026-09-18) is now a pointer to `CLAUDE.md`. Review findings folded into the new map: the protocol list now names every dispatched message type (the old table omitted `delete_session`, `reconcile_self`, `resolve_contradictions`, `persona_preview`, `get_prompt_deltas`, `set_reflection_feedback`, the `til_*` / encounter / prompt-experiment messages); `_resolve_max_new_tokens` lives in `core/generation.py`, not `server.py`; ten prompt files named in code are default-written and untracked (`assoc_*`, `prompt_{pattern_cluster,rewrite,consensus,choice}`, `select_prompt.txt`, and on this checkout `ask_origin`, `facts_decision_block`, `facts_reread_block`), which the map now says. `MODULE_NOTES.md` is exported to the public copy (the map itself never was); the publisher's docs-README pointer rewrite was updated to the new wording. No runtime change.

- **A cleared HF cache left the box refusing to download its own base model** (`core/fast_load.py` `snapshot_is_cached`/`adapter_base`, `core/inference_backend.py` `load`/`_is_hub_offline_error`). `hf_offline_if_cached` sets `HF_HUB_OFFLINE` for a load whose snapshot is already on disk (84 Hub HEAD requests, ~12 s, per load — SPARK_LOADING.md). It was pointed at `load_name`, which with an adapter present IS the local adapter dir, and `os.path.isdir` made that trivially "cached" — but unsloth resolves the base from the adapter's own `adapter_config.json`, so with the cache cleared the guard turned a missing 18 GB base into `LocalEntryNotFoundError` and a deliberate exit 1, repeated on every relaunch. Two fixes, because the check can only see the id it is handed while the loader resolves more behind its back (an adapter's base — now followed — and unsloth's bnb-4bit twin of a named repo, which is not knowable from here): `snapshot_is_cached` follows a local LoRA dir to its base and reports the pair, and `load` retries ONCE with Hub access when an offline attempt dies on a missing cache entry (`LocalEntryNotFoundError` anywhere in the cause chain, so a real OOM or a corrupt shard still raises). **The policy, stated where it was only implied: skipping Hub round trips is an optimization and must never be why a load fails.** Verified on the box — `hf_offline=False`, 18 GB fetched, model loaded.

- **Sleep lifecycle review fixes** (an external code review of the reflection/deliberation lifecycle; `core/reflection_staging.py`, `core/reflection_runner.py`, `core/reflection_service.py`, `core/reflection_config.py`, `core/deliberation.py`, `server/reflection_run.py`, `client/ui/sleep_widget.py`). Seven findings, all reproduced against the code. **(1) Checkpoint recovery lost gists and protocols:** `fold_checkpoint_to_live` decided "already frozen live" per FILE, and `iter_chat_products` yields every `.state.json` first — so after copying a chat's state sidecar (which carries `reflected_at`) it skipped that same chat's `.summary.json` and `.facts.json`, then discarded the checkpoint. The decision is now made once per stem before any copy. Any run interrupted and recovered before this lost those two files for every recovered chat; a re-reflection (Chat tab → *Re-reflect chat*) regenerates them. **(2) Deferred clean-base work deleted before it ran:** a `chat_reflected` chat's pending judge/fact payload was deleted at load and the chat frozen `reflected_at` + checkpointed before the clean-base phase — a stop before that phase froze the chat forever with the work lost. The load and the delete are now separate seams (`consume_pending_clean_base_fn` / new `ack_pending_clean_base_fn`), and the freeze, checkpoint and ack all land after the clean-base batch (runner step 4b); on a stop the chat stays `chat_reflected` for the next run. **(3) `completed` before commit:** the runner finalized the run `completed` / `mutations_applied: true` before the service merged memory, committed sidecars, produced the persona or handed off training; the client stopped polling there and `request_stop` was refused during those stages. `execute_run` gained `finalize` (default True — dry runs, the CLI and the background pass are unchanged) and returns the outcome; the service passes `finalize=False`, the runner leaves the run `running` in phase `downstream` with its summary attached, and the service writes the terminal status. A stop that lands during the downstream stages leaves the commit (done, safe) and withholds only the train hand-off (`training_handed_off: false`). **Interpretation change:** a run record's `completed` now means the downstream stages ran; earlier records could read `completed` with a failed commit behind them. **(4) No admission reservation:** `_reflection_run_active` flipped only when the executor began the run, so two requests behind a busy executor were both queued, and same-second requests got one run id (the store overwrote the first record). The handler now reserves the flag before any side effect (every early return releases it) and suffixes a colliding id. **(5) Deliberation dispatched off an unclosed think:** `_parse_decision` ran on a generation cut inside its reasoning, where an `ACTION:` match is deliberation about the options, and dispatched it (observed: `wander`). The shared `reasoning_text.truncated_before_answer` guard now refuses it — `nothing`, `cut_before_answer`, no dispatch, no decline. **(6) The headless CLI had diverged:** it read the retired `inference/data/hot/chats` (empty since the corpus moved) and its own copy of the staging stages committed `*.state.json` only, stranding every staged gist and protocol; it now resolves chats through `training.reflections_path.hot_chats_dir()` and wraps the shared `core.reflection_staging` stages. **(7) A temperature change silently replaced the token cap:** the Sleep tab sent `max_new_tokens_setting: "75%"` with any non-default temperature, and the server's parser defaulted an absent value to the same — 45k tokens of pre-allocated KV cache per pass on a 60k window, the allocation the runner's fixed `8192` cap exists to avoid. The client sends no cap and `reflection_config.DEFAULT_MAX_NEW_TOKENS_SETTING` ("8192") is now the one default the runner and the parser share. Also fixed in passing: `run_stage_apply`'s candidate-adapter promotion imported `reflections_path` bare, which is on no path (`training.reflections_path`). Not done: the review's proposal of one lifecycle controller owning admission, cancellation, commit and completion — the terminal-status ownership above is the first step toward it.

- **The deliberation executive stops resting forever** (`core/deliberation.py`, `deliberation_prompt.txt`, `server.py` `_wander_budget_note`). Observed under `deliberation.mode: sole`: wander stopped entirely with reading budget unspent. Her CoT weighed every option against the last thing on the worklog — a pivot she had raised — and found each redundant with it or waiting on its reply, wander included ("no new external material introduced recently"), so she chose `nothing`; a rest records no episode, so the next hour's context was byte-identical, undated, and the same reasoning held at hour nine as at hour one. Three fixes, all in the context so the prompt file stays overridable: every worklog line and open thread now carries how long ago it happened, with the newest's age in the header; a process-local rest streak (`_rest_streak`) states how many consecutive rests and since when; and `configure(note_fns=…)` renders a *Where things stand* block, wired to the wander budget so "N wanders unspent" is in front of her rather than a `no_wander_budget` skip after the fact. The prompt (file + `_DEFAULT_PROMPT`) now groups the options by whether a person hears from her — wander and revisit are private, the rest may end in a message — and says that silence owed to a person is silence toward THEM, not idleness. `_record_dispatch` requires `decided` and counts rests; any dispatched action resets the streak. Self-test covers ages, the streak, the notes and their presence in a real pass's context.

## 2026-09-20

- **Outreach stops fixating on an ask it cannot decide, and the executive sees what its dispatches came to** (`core/outreach.py`, `core/deliberation.py`, `server.py` `_outreach_consumed_interval`, `config_schema.py`, Sleep-tab messages). Observed under `deliberation.mode: sole`: the executive chose `reach_out` hour after hour, outreach picked the same long meta ask every time (`surfaceable_questions` orders fewest-raised-first and a truncated attempt writes no `surface` op, so nothing rotated), the decision pass hit its 8192 cap inside its think every time (the window three quarters empty), and nothing else ran. Three changes. (1) The decision budget is `outreach.max_new_tokens` (default 12288, clamped by `runtime_state.reflect_output_reserve` leaving 4096 for the input — the shape check-in and synthesis already had). (2) An ask whose attempt produced no decision (`truncated` / `opener_leak`) is held out of `_pick_ask` for 24 h, process-local by design: the durable stores each record something the user saw and this records nothing they did; `all_recently_failed` names a walk emptied by the hold alone. (3) The executive's context carries a ring of its last six executing dispatches and what each came to (`came to nothing (truncated)`, `not run (no_handler)`, `ran`) — a failed drive records no worklog episode, so without this the next deliberation had no evidence the last one failed. Shown, not folded into her record. `opener_leak` now spends the outreach interval like `truncated` (it ran the generation; the classifier retried it every poll).

- **A user-deleted chat closes its reach-out thread at once** (`core/chat_worklog.py` `close_removed_session` new, `expire_stale_reachouts` reordered; `core/session_ops.py` `handle_delete_session`). Deleting an Ava-initiated chat from the Chat tab is how the user declines that conversation, but the worklog thread it opened stayed open: the sweep noticed a session gone from disk only behind the same 48 h age gate as an ignored opener, so the deliberation executive read it as a loose thread awaiting a reply for two days and waited on a chat that no longer existed instead of reaching out anew. The delete handler now closes every thread keyed on the chat at the point of effect (naming who took it down and that it went unanswered — information for the next decision, not silence: no reach-out tombstone, no backoff), and the sweep closes a missing session at any age before the gate. `session_deleted` carries `threads_closed`. Existing worklogs need no migration — the next sweep closes any thread already in that state.

- **Prompt rewrite review fixes** (reviewed baseline `d94941c`; locator `6807701`, patterns `638ae1c`, event `9f72b01`). Revert now interrupts an autonomous rewrite, and activation checks the incumbent text and experiment timestamp under the same lock as Revert. Operator changes close active or resumed attempts as `cancelled`, consume no evidence, and stamp the normal cooldown instead of immediately restarting. Decision-only deliberation previews no longer record declines. Prompt clustering accepts large recurring groups while persona/fact callers retain their default oversized-group guard; adding a sixth corroborating chat no longer splits a mature pattern into singletons. The locator and persona preview resolve the active experiment before the seed. Existing stored patterns/deltas are not migrated: grouping corrects on the next normal fold, and future locator passes read the live prompt. GPU-free regressions cover concurrent Revert, activation/resume fences, same-text edits, preview dispatch, multi-block recurrence, and active-prompt fallback.

## 2026-09-19

- **Prompt tab: the autonomous rewrite's window** (`core/prompt_rewrite.py` `status_report` + `prompt_rewrite_status`, `client/ui/prompt_widget.py`): a third pane shows the currency gate condition by condition (offered or the first failing reason, gap hours left, deltas since the last attempt, unspent patterns, the line the executive would be shown), the budget as the last fold left it, and the attempt log newest-first with timestamp, outcome, chosen candidate, WHY and — for the newest attempt — every candidate's text. `offer()` gained `check_model` so the chain is readable with no model loaded.
- **Prompt rewrite stage 3: the event** (`core/prompt_rewrite.py` new, `core/deliberation.py`, `core/generation.py`, `core/prompt_experiment.py`, `core/worklog.py`, `server.py`, `config_schema.py`, `client/ui/prompt_widget.py`; PROMPT_REWRITE.md §4–§7): a `rewrite_prompt` deliberation action, conditionally offered through an injected `offer_fn` when the pattern budget clears, the last attempt is ≥ `prompt_rewrite.min_gap_hours` (24) old and a delta was logged since. The body: `samples` (5) drafts of the LIVE prompt at 0.9 with the mature patterns as evidence, a consensus pass over them, one final draft around the notes, then a blind shuffled choice among the incumbent, the drafts and the final (adapter, `WHY`, optional reversed-order check). *Changed* replaces the active experiment through `prompt_experiment.save_experiment` (seed carried as the revert anchor), consumes the patterns' delta keys, and opens a first-person `prompt` worklog thread; *stayed* and *declined* spend nothing and only stamp the gap. Every completed generation is persisted under `data/hot/prompt/rewrite/<event>/`, a chat turn preempts the event exactly as it preempts background reflection, and the next dispatch resumes from the last completed step. The gate reads the pool through `rewrite_log.jsonl` rather than `patterns.json`, which is rebuilt only by the next reflection run. The Prompt tab shows `set by Ava herself (rewrite event …)`; Revert there is the veto. Shipped OFF and **flipped ON the same day** (`prompt_rewrite.enabled` default true — the owner's call: run it live and adjust, never disable; the schema back-fill writes the default into a box's config on its next boot, so a box that pulls is live at once). Worklog kinds gain `prompt` (and the `aha`/`pivot` kinds assoc_bridge was already writing, which the kind list had been coercing to `conversation`).

## 2026-09-18

- **Prompt rewrite stage 2: patterns + budget** (`core/prompt_patterns.py` new, `core/reflection_runner.py`, `core/reflection_config.py`, `core/deliberation.py`, `config_schema.py`; PROMPT_REWRITE.md §3/§4): every normal reflection run folds the locator's unconsumed deltas into patterns inside the clean-base batch (task (e), after fact dedup) — grouped per `SCOPE` then by meaning through `persona_cluster.map_reduce_groups` with its own default-written prompt, one vote per distinct chat weighted `recency × (0.5 + tension_rank)`, NO tenure discount (that exists for the persona's circular self-vote; a delta has no such loop), mature at `prompt_rewrite.maturity` (1.9) over `min_chats` (2) — and writes `data/hot/prompt/patterns.json`. The deliberation executive reads that file and is shown the patterns in its context; no action spends them yet. Consumed keys are read from `rewrite_log.jsonl`, which the stage-3 event will write. Run override `prompt_patterns`. One bug caught by the self-test: consumed keys must be dropped AFTER the newest-per-exchange fold, or a superseded older delta resurfaces the moment its successor is spent.
- **Prompt rewrite stage 1: tension baseline + locator** (`core/tension_baseline.py` new, `core/prompt_mutation.py`, `core/reflection_runner.py`, `prompts/prompt_mutation_prompt.txt`, `config_schema.py`; PROMPT_REWRITE.md §2/§8): the prompt-mutation pass, which ran on `revise` exchanges only, also runs on a KEPT exchange whose CoT tension ranks at or above `prompt_rewrite.tension_percentile` (0.8) against a corpus baseline — percentile within (model, adapter, reply language), falling back adapter- then model-wide below 30 samples, no baseline ⇒ the old revise-only behaviour. The first real-corpus measurement changed the design's metric set: the stored `median_entropy` and `median_margin` are constant on every segment, so the rank is the mean of directional percentiles over `peak_entropy`, a derived `mean_entropy`, `contested_frac` and `p10_margin` (18 of 388 kept exchanges clear 0.8). The prompt file gained a `{locator}` slot (a kept reply must not be framed as a drift); each delta record carries `locator` / `tension_rank` / `tension_key`, the fields the stage-2 pattern fold will weight by. `interlocutor:"ai"` transcripts are skipped in both cells. Still logged-only; nothing rewrites the prompt.
- **`assoc_feed` waits an hour while a client is connected** (`server.py`, `core/idle_scheduler.py`): the feed fired on the 5-minute idle window that suits the GPU-free upkeep jobs, but it holds the GPU lock for minutes and a chat turn does not check that lock — it queues on the single executor thread behind it. The idle clock counts SENT turns only, so a user composing a long reply looked idle, the feed started, and the reply landed minutes late. New gate (`_assoc_feed_ready`): with no client connected the 5-min window stands (nobody can be typing); with one connected the feed needs the drives' full idle hour; once fired it re-arms every 15 min instead of hourly, so a long absence keeps the library within minutes of the corpus (an unchanged corpus is a cheap `current` bail). "Skip while connected" alone was rejected — this box's client is routinely left open all day. Also: `IdleJob._last_ran` now defaults to `-inf`, not `0.0` — `time.monotonic()` is uptime on Linux, so after a reboot every hourly job's first wake waited out the rest of that hour, and the scheduler self-test failed on macOS, where the clock starts near zero.
- **Correction to the documentation audit** (code checked at `a97865c`): the inference entry point remains `server/inference/server.py` (the watchdog launches it with that working directory); only the box configuration moved to the server root. The training sequence cap defaults to 8192 in both `train_cycle` and the configuration schema. Corrected the prior audit’s mistaken entry-point path and 4096-token default; no runtime change.

- **Documentation reconciliation, no runtime change** (audited code `9ecb488`; executive default `1e931b8`, swap recovery `38664fb`). The operator confirms that implemented code has been exercised on the GPU box and is operationally sane. Moved working executive/worklog, gossip/API, and retrieval features out of Pending/Logged-only where their table placement contradicted their implementation; corrected the standing portrait, weighted judge gate (two themes ≥1.9), polarity/counter feedback, map-reduce clustering, retired explicit persona CoT injection, and current training defaults. Historical plans are explicitly distinguished from the current contract. README manifests and the frozen legacy design are unchanged.
- **Adapter validation is deliberately off.** The disposable adapter is an output of the process; a failed live result is evidence for adjusting that process or its corpus. Removed suggestions that a missing behavioral gate is an implementation defect or required redesign. Technical row checks remain active; optional controlled process comparisons are separate from adapter acceptance.
- **State/evidence contracts narrowed to what the code captures.** `snapshot_state.py` omits the associative library’s generated records/access history and graph aliases, and its manifest omits the associative embedder and immutable model revisions. A temporary fixture confirmed the export omission at `9ecb488`. Forensic capture is best-effort and scratch rows can be deleted after capture failure; early failures need not be recorded. Documented these as open implementation gaps, distinct from the working GPU/export paths. Live data repairs also mean historical snapshots can contain unique evidence; a fresh rebuild does not reproduce overwritten historical inputs. The corpus fingerprint does not cover full message prefixes or LR multipliers.
- **Historical failures are not current-corpus measurements.** Kept the corrected RAG-pollution post-mortem with its attribution limit; removed stale poisoned-corpus counts as grounds for current repair work. Corpus growth remains a scaling question, not an assertion that current training is degrading.

## 2026-09-17

- **Every documented config knob is back-filled into `server_config.json` at boot**
  (`server/config_schema.py`, `server.py::_backfill_server_config`, `settings.py`). The
  settings editor's schema (tabs → fields → defaults) moved out of the PyQt file into a
  stdlib-only module that the server loads by path, and the boot back-fill — until now
  three hand-listed training keys — walks that schema and writes each ABSENT knob with
  its default. The schema grew to cover what only `cfg.get("x", default)` calls had
  documented: check-in's per-person caps and budgets, reach-out pacing (outreach /
  synthesis / revisit / stale-opener sweep), the deliberation block, the retrieval
  kill-switches and the reflection-block subject cap, the facts tree, the associative
  library, and logging — 114 fields, 65 of which a box configured through the old editor
  gains on its next run. Two contracts make it behaviour-preserving: a written value is
  the one the reader would have used anyway (the self-test cross-checks the import-light
  readers; the editor's `chat_repetition_penalty` default was wrong — null, where the
  reader's absent-default is 1.1 — and is corrected), and a null whose meaning differs
  from absence is never written (`model_id`/`adapter_id` are box state and never
  written; a key present at any value is left alone). Interpretation: a
  `server_config.json` from before this date lists only what someone set; one after it
  lists every knob the box knows, at the value the box was already running with.

- **The deliberation executive is the default chooser** (`core/deliberation.py`
  `DEFAULT_MODE`, `settings.py`, `server.py`). `deliberation.mode` now defaults to `sole`:
  on a box with no explicit `deliberation` block, the reach-out family (outreach /
  synthesis / check-in / wander / aha / pivot) is no longer registered on its own hourly
  timers and runs only when Ava chooses it from her worklog; the upkeep jobs keep their
  clocks. `shadow` — the mode the executive shipped in three days earlier, executive AND
  timers side by side — stays available but must be set explicitly. Interpretation: an
  Activity journal from a default box before this date shows timer-driven reach-outs
  interleaved with the executive's choices; after it, every reach-out on a default box
  traces to a `Deliberation: chose …` line (or a manual button). An unknown mode now falls
  back to `sole`; an unreadable config still yields `off`.

- **A failed clean-base swap no longer leaves the box model-less; the load placement is
  pinned on every single-GPU box** (`core/agentic.py`, `server.py`, `core/unified_memory.py`).
  A reflection run's ingestion phase swapped to the clean base and the load was refused by
  the offload guard (`lm_head -> meta`) with 28.1 GiB free — the same free VRAM every
  earlier swap on that box had loaded at, so the planner's estimate was marginal, not the
  memory. The restore then ran INSIDE the `except` block, where Python 3.10's thread
  exception state still pinned the failed load's frames and their ~20 GB of weights; it saw
  10.4 GiB free, was refused the same way, and the run failed with "restart the server and
  retry" — the process sat with no model (20 GB reserved, 2 GB allocated) until an operator
  noticed hours later. Three changes: the restore runs outside the except block, after the
  traceback chain is severed and a real reclaim, and retries once; a second failure calls an
  injected model-loss hook that journals the loss and asks the watchdog to restart the
  process onto the config's `adapter_id` (the last known good adapter) after a 10 s grace;
  and `unified_memory.load_kwargs` pins `device_map={"": 0}` on any single-GPU box, not
  only unified memory — on this project an offloaded load is a failed load regardless, so
  the planner's conservatism only ever converted a fitting model into a refusal
  (`AVA_LOAD_PLACEMENT=planner` opts out). Interpretation: a `run_failed` reading "Model
  lost … a watchdog restart … has been requested" is the hook firing; an earlier run
  reading "Restart the server and retry" predates it.

## 2026-09-14

- **The deliberation pass becomes the executive, in shadow mode** (`core/deliberation.py`,
  `server.py`, `prompts/deliberation_prompt.txt`, `client/ui/worklog_widget.py`,
  `settings.py`). Since 2026-07-24 Ava could read her worklog and say what she would do
  next, and nothing acted on it; meanwhile the box grew to nine autonomous impulses on
  independent clocks (outreach, synthesis, check-in, wander, background reflection, then
  the assoc aha and pivot, plus the upkeep jobs), coordinated only by the reach-out gate,
  which throttles messages, not intent. The drives are now defined ONCE in `server.main()`
  (`_drives`) and handed both to their own IdleJob and to the executive's injected
  `dispatch` map, so a choice runs the same body a timer would; `check_in`, `aha` and
  `pivot` join the action set; `revisit` — a reflection run, i.e. a coroutine that
  dispatches to the same executor — is scheduled headless (`_send` now tolerates
  `ws=None`) and returns at once. `deliberation.mode` in `server_config.json`: **shadow**
  (default, and what runs now) keeps every per-drive timer AND runs the hourly executive,
  so the Activity tab shows what she chose beside what the timers did; **sole** stops
  registering the six drive timers (`CHOSEN_JOBS`) and leaves her the only chooser, the
  upkeep jobs untouched; **off** keeps only the Worklog tab's button, which stays a dry
  run. Her decision is journalled (`Deliberation: chose wander — <her why> → wander
  ran`) but deliberately not worklogged: the drive records its own episode, and an hourly
  "I decided to rest" would fill the window the next deliberation reads. A decision spends
  the interval whatever she chose; a no-model/no-history bail retries. Self-test covers
  the mode knob and the dispatch seam with stubs (no handler → reported; a raising handler
  → its error; `nothing` never dispatches). Interpretation: from here the timers are the
  baseline and her choices the experiment; the `sole` flip is a config edit once the
  comparison reads well.
- **Every "untested on a live GPU" / "not yet exercised" caveat in this file and in
  `AVA_STATUS.md` is discharged** (docs only, no code). Those sentences were written the
  day each subsystem landed, before it had run on the box; each has since been exercised
  on the DGX Spark in ordinary operation — reflection runs, training cycles (the adapter
  lineage under `server/models/`), the background per-chat drain, the activity journal,
  the portraits, the facts protocols and their fetch channels, the reach-out jobs, the
  public API, the Training-review repair flow. The status table no longer carries the
  caveat; the older entries below now read *"since exercised on the GPU box (see the
  2026-09-14 entry)"* where they said *"not yet"*, so a reader of the history is not left
  with a warning the code has outgrown. Gossip included: the driver ran end-to-end between
  the main box and a second box, which is what the second box was brought up for. Two scopes are stated
  rather than hidden. **Dense models only** — every exercised claim is on gemma-4-31B;
  the two MoEs of 2026-09-07 ran as a vanilla model — as Ava on the bare base with no
  adapter, and as the Encounter counterpart (gpt-oss-120b and Qwen3.5-122B-A10B answering
  Ava's turns) — but training was never attempted on an MoE, so no adapter has been fit
  on one, and whether MoE should be supported at all is an open question
  (`AVA_OPEN_PROBLEMS.md → MoE On A 121 GB Box`). **Two avatars, one operator** — the
  multi-person paths (per-person portraits, check-in's per-person sweep, the hearsay gate,
  `AVA_MEMORY.md`'s discretion norm) were exercised with the operator playing two distinct
  people, so the mechanics ran on real data while the privacy stakes of a genuine second
  person were never in play.

## 2026-09-12

- **A safety refusal in the associative library's relation pass no longer erases a
  document's relations** (`assoc/relations.py`, `server/inference/core/assoc_bridge.py`,
  `assoc/bench/test_relations.py`). The training box's journal at 2026-09-11 23:46 UTC holds an
  `assoc_relations` body that is the Gemma base model's boilerplate refusal (helpful and
  harmless assistant, guidelines prohibit sexually explicit content) instead of `n:
  pred(a, b)` lines. The pass runs on the loaded model with the adapter, but thinking
  OFF, greedy, under a bare labelling prompt — so the base's safety reflex answers in the
  first tokens, in a register chat never shows because chat reasons inside Ava's frame
  first (it read to the operator as a different, smaller model; it is not). A refusal
  parses to zero lines, and zero lines were cached as "no relation" for every claim of the
  batch (up to 60, the whole document) under the current prompt version — never retried
  unless the prompt file changed. The claims and their facts survived (the witness had
  produced 21/2/14 facts on the same chats three minutes earlier); what was lost were the
  typed edges, i.e. the `asked_about` / `wants` / `looking_for` needs the aha reads. Now an
  empty answer is classified (`is_refusal`: a strong safety vocabulary, or a weak "I cannot
  fulfill" that does not go on to talk about the claims — the model's observed way of
  explaining a genuine none), a refusal is retried ONCE with thinking on and a larger
  budget, and a batch still refused is left uncached and counted (`refused` /
  `refused_claims`, named in the feed headline) so the next wake tries it again. A real
  none is cached exactly as before. Interpretation: across 252 relation-pass bodies on the
  box, one is a safety refusal and a dozen are verbose nones. Five bench checks pin the
  cache behaviour.
- **The same gap closed in the LLM witness** (`assoc/refusal.py`, `assoc/witness.py`,
  `assoc/bench/test_witness_refusal.py`). No witness refusal has been observed, but the
  mechanism was identical one layer up and its cost higher: a refused group parsed to
  zero facts, counted as a group that established nothing, and `run_witnesses` wrote the
  protocol over the imported one — Ava's `chat_facts` record for that chat — with those
  chunks empty, for good, since the feed never re-imports over the library's own
  protocol (`kept_own`). The classifier moved to `assoc/refusal.py` (one definition for
  both passes; the relation pass keeps its `RELATIONS:`-stripping wrapper). A refused
  group is retried once with thinking on (8192 budget); still refused, it is reported
  (`refused_groups` / `refused_chunks`, in the witness headline) and the document's
  previous protocol's facts for exactly those chunks are kept, marked `fallback` with the
  witness that produced them — chunk ids are structural and shared across versions, so
  the previous anchors name the chunks this run refused. Interpretation: a protocol
  carrying `fallback` facts is a mixed record (the library's witness for most chunks,
  Ava's for the refused ones), extracted and complete, so the hourly witness does not
  re-refuse it every wake; a document with no previous protocol reports the refused
  chunks with nothing to fall back to. Six bench checks.
- **A refused document is re-witnessed once per NEW adapter, never per prompt edit or
  per wake** (`assoc/witness.py` `pending_documents(current_model=…)`, `assoc/library.py`,
  `core/assoc_bridge.py` `witness_model_id`, `server.py`). The operator's point: the
  safety reflex lives in the base weights, a prompt is only what it fires against, and
  the adapter — refit from scratch on the frozen base each build, over a corpus that
  includes the very chats the reflex fires on — is the one thing on the box that moves
  those weights. So each new adapter is a fresh draw, and the previous rule (re-queue on
  prompt version) was keyed on the wrong thing. Three parts: the protocol now carries
  `refused_chunks` at top level beside its `model_id`; the bridge records the LOADED
  adapter's name as that `model_id` (it was the constant `"ava"`, so no protocol on the
  box could tell adapters apart — every protocol written before this carries it and
  will read as "a different model" exactly once, which is the intended first retry);
  and `pending_documents` re-queues a document whose protocol carries refused chunks
  under a `model_id` other than the current one. A never-refused document ignores an
  adapter change; a document still refused under the new adapter lands back on the
  fallback with the new name recorded, so it is not retried until the next build. The
  relation pass needs no such rule — a refused batch stays uncached and is re-sent
  every wake, so the first wake after a new adapter is its retry. Nine bench checks in
  `test_witness_refusal.py`, three in the bridge self-test.

## 2026-09-11

- **"[Errno 61] Connection refused for a few minutes" was the client's resolver, not the
  server** (`client/core/backend_client.py`). The activity journal showed the server up
  and generating through the operator's refused connects, and a live probe from the
  client box showed why: `<server>.local` resolves by mDNS, and macOS answers with
  EXPIRED cache entries first — an IPv6 link-local (the server binds 0.0.0.0, IPv4 only)
  and `192.0.2.20`, an address the training box no longer has and another machine
  now does — with the fresh `192.0.2.10` arriving ~0.7 s later. A lookup inside that
  window offers only refusing addresses; `socket.create_connection` tries them and
  raises the last refusal, and every quick retry does the same. `BackendClient.connect`
  now opens the TCP socket itself (`_open_socket`, handed to websockets via `sock=`):
  every resolved address, IPv4 first and the last-good one before that, re-resolving
  and retrying for 12 s when all refuse. Interpretation: an mDNS name that also carries
  a stale address is a client-side condition; the sidecar/watchdog HTTP calls (urllib)
  share the exposure and are not yet routed through the same helper. The operator can
  also clear it at the source (a DHCP reservation for the server, or flushing the Mac's
  resolver cache).
- **Silent inference-server deaths are now reported and self-healed; a booting server
  no longer refuses connections** (`server/watchdog.py`, `server/inference/server.py`,
  `core/runtime_state.py`, `client/ui/chat_widget.py`, `client/ui/main_window.py`).
  Found while chasing the refusals above and real on its own: the training box's server.log held
  four relaunches with no watchdog line before them — the process had died silently (no
  Python traceback: a native crash or a SIGKILL), three of them right after a RAG-engine
  (MiniLM + faiss) build and one inside a background reflection drain. The watchdog had
  no relaunch-on-death, so each death left the server down until the operator pressed
  Restart, and the model load after that refused connections too. Three changes: (1) the watchdog gains a **crash reaper** — the child's returncode is written
  into server.log with its meaning (-9 = OOM killer, -11/-6 = native crash) and the
  server is relaunched, with a crash-loop guard (3 in 10 min ⇒ stays down) and no
  relaunch on a deliberate exit code; `GET /status` carries `last_exit`; (2) the
  inference server enables `faulthandler` so a native crash leaves a thread dump, and
  **listens before it loads** — the startup auto-load runs on the executor after
  `serve()`, `status` says `loading: true`, `load` is refused meanwhile, and the client
  is pushed `loaded` when the model lands; (3) the client follows that push
  (`sync_model_state` on the 2 s timer) and shows "loading model…". Interpretation: a
  "Loading model" line in server.log preceded by neither a watchdog job/pull line nor
  a reaper line is, from now on, impossible — every relaunch names its cause. The root
  cause of the crashes is NOT yet identified (the kernel log is out of reach from the
  client box); the next one will carry a faulthandler dump and a signal name. The
  watchdog change needs a watchdog restart (SSH) to take effect.

- **The assoc feed's sync + rebuild run in a subprocess** (`server/inference/core/
  assoc_feed_worker.py` (new), `core/assoc_bridge.py`, `assoc/rebuild.py`, `assoc/library.py`,
  `assoc/store.py`, `assoc/ava_import.py`). Symptom: after the library was wired in, the
  server "refused" connections while `assoc_feed` ran — a WebSocket opening-handshake
  timeout (10 s default on both ends; the server now serves with `open_timeout=None` and
  the client connects with 300 s, commit `a619797`). Cause: a rebuild is minutes of pure-
  Python work (lemmatization, codebook, dedup, PageRank) that never releases the GIL, where
  a generation spends its time in CUDA kernels that do — so on the executor thread it
  starved the asyncio loop for the whole wake, and the feed fires 5 min into idle, i.e.
  exactly when a user returns. The library never loads the LLM, so `feed_sync_and_rebuild`
  now runs in `python -m core.assoc_feed_worker` (paths + raw config on stdin, progress
  relayed through the parent's stdout, report as a marked last line, killed after
  `assoc.feed_timeout_s`); the parent blocks on the pipe, GIL released. The one model-bound
  piece, the typed-relation pass, is factored out of `rebuild()` into
  `assoc.rebuild.relation_pass` (cache-only with no `generate_fn` — cached relations now
  survive a rebuild that cannot generate, which they did not before) and runs in the parent
  AFTER the worker's rebuild, over the build it produced; the next wake's rebuild folds the
  cache (forced when the cache holds relations the build lacks). Interpretation: a
  relation reaches the edge table one wake later than before; `Library.reload()` /
  `Store.reload()` exist because the store held `keys.json` in memory from construction
  and a pull over a build another process wrote found nothing allowed. Two bugs found by
  the new self-test checks and fixed alongside: `ava_import.sync` never saved the
  fingerprint of a chat the library had witnessed itself (`kept_own` imports no protocol,
  and the state was written only when a protocol was pending), so such a chat was re-
  ingested on every wake; and the relation pass mutated the loaded build's claims in place,
  so the in-memory build claimed relations the on-disk edge table did not carry.
  `assoc.feed_subprocess: false` restores in-process.

## 2026-09-10

- **The associative library is wired into Ava, off by default** (`server/inference/core/
  assoc_bridge.py` (new), `core/modules.py`, `core/generation.py`, `server.py`,
  `core/state_wipe.py`, `assoc/ava_import.py`). `assoc/` (ASSOCIATIVE_MEMORY.md, milestones
  1–4 landed 2026-09-09, measured on Ava's real corpus 2026-09-10) now sits beside the
  facts-tree channel: an `assoc_feed` idle job keeps a derived library under `data/assoc/`
  fed from the same transcripts, texts and `.facts.json` protocols the reflection passes
  write (nothing generates; the library imports the protocols and rebuilds); an
  `assoc_fetch` Modules-tab entry runs its stage-1 select on one chat beside `fact_fetch`;
  and `assoc.enabled` hands the live turn's stage 1 to it in the tree channel's own result
  shape. Interpretation: `data/assoc/` is derived and disposable (wiped with regenerable
  state, not snapshotted); the facts block's `channel` field says which store answered a
  turn; the library's `Policy` withholds `person:_self` exactly as §10 does. The switch
  stays off until the workbench comparison says otherwise.
- **The aha reaches the reach-out surface** (`core/assoc_bridge.py`, `server.py`,
  `assoc/aha.py`, `assoc/library.py`). An `assoc_aha` idle job turns every open `[ask]`
  into a standing need of the associative library, takes what the feed brought in since
  the last wake as the stimulus, runs the library's `aha()` arithmetic and one thinking-on
  judgement per wake, and on `connect`/`satisfies` writes an opener through outreach's own
  session writer (`ask_kind:"aha"`) behind the shared reach-out gate, stamping the ask
  surfaced and opening a worklog thread. Interpretation: a session with
  `initiated_ask.ask_kind == "aha"` was opened by a connection the library found, not by
  the outreach decision pass; the library's `state/aha.jsonl` ledger is why the same
  connection is never raised twice. Registered needs (no claim behind them) are new in the
  library: anchored by what their text retrieves, benched in `bench/test_aha.py`.
- **The library witnesses her chats itself** (`core/assoc_bridge.py`, `server.py`,
  `assoc/prompts/witness_chat.txt`, `assoc/witness.py`, `assoc/relations.py`,
  `assoc/rebuild.py`, `assoc/ava_import.py`). The real graph showed the aha starved by the
  `chat_facts` protocol (abstract-noun entities, no relations, the assistant's prose copied
  in the first person), so an `assoc_witness` idle job now runs the library's own rewritten
  chat witness over her transcripts, newest first, three per wake, replacing the imported
  protocol; the feed never imports Ava's protocol over a document the library witnessed
  itself; the typed-relation pass rides the feed's rebuild with a per-wake budget that is
  not part of the build's version. Interpretation: in `data/assoc/`, a chat protocol with
  `witness: "llm"` is the library's reading and one with `prompt_version: "imported"` is
  Ava's; `chat_facts` on Ava's side is unchanged and still feeds the facts tree. Measured
  on three real chats (gemma-4-31B): abstract entities and first-person copies gone,
  `asked_about` relations appearing, 25–30% fewer output tokens.
- **`assoc.enabled` defaults to true** (`core/assoc_bridge.py`, `core/generation.py`). The
  operator asked for a pull-and-go default on the main box; a library with no build yet
  (before the first `assoc_feed` wake) falls through to the facts-tree channel, so the flip
  never costs a turn its facts. `assoc.enabled: false` restores the tree channel exactly.
- **The pivot reaches Ava** (`core/assoc_bridge.py`, `core/modules.py`, `server.py`,
  `assoc/senses.py`, `assoc/lex.py`). An `assoc_pivot` idle job runs ASSOCIATIVE_MEMORY.md
  §6 in its `wander` mode: from the newest conversation with a protocol, a word with
  another sense on record (sound and root bridges by config), the far side rendered, and
  one thinking-on pass deciding raise / keep / nothing — a raise is an outreach with
  `ask_kind:"pivot"` behind the shared gate, a keep is a `pivot` worklog episode whose
  first-person summary is, for the first time, written by a generation. Interpretation:
  a worklog entry of kind `pivot` and a session with `initiated_ask.ask_kind == "pivot"`
  came from a word, not from a need. The library's sound bridge gained two rules the real
  vocabulary forced (never a word's own inflection; a same-stem pair is the root bridge's)
  and both stoplists grew (`since`, `самый`).
...
- **The reach-out passes look at the record before they raise a question.** Outreach's
  ...
  block, never the decision. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **The facts tree reaches a live chat turn — on by default.** `graph.enabled`
  ...
  **On by default.** Since exercised on the GPU box (see the 2026-09-14 entry); at the time everything verified was the
...
- **Unanswered reach-outs are deleted from the corpus — and the record that she was
  ...
  The sweep is GPU-free; the self-tests cover it. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **An outside view of herself: `[self_impression]` and the `_self` portrait.**
  ...
  **Nothing injects this portrait, and nothing feeds it back into the digest.** Both are
  ...
  to end; since exercised on the GPU box (see the 2026-09-14 entry).
...
- **`UNSLOTH_CE_LOSS_TARGET_GB` raised `0.5` → `1.0`** (`training/train_cycle.py`), spending
  ...
  **Interpretation for a future reader: this is a bet on headroom, not a validated ceiling.**
  ...
  before and after are comparable. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **Facts tab — operator cleanup of the live `[fact]` set** (commit `443376d`), the counterpart of the Persona
  ...
  **of that kind** (a reflection that landed since the fetch is a rejected conflict, not a
  ...
  the RAG evict is what does the work. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **`main()`'s own local variables pinned the startup model in VRAM for the life of the
  ...
  **two** full models on the card. On this box that is 18 GB twice against 31 GB, so the
  ...
  the real cause. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **A model could load successfully and be incapable of generating** (commit `3416218`). Reported as
  ...
  **Not changed:** `handle_load` still does not evict an already-loaded model before
  ...
  decision. The guard's device-map/meta classification was exercised against synthetic
  maps; since exercised on the GPU box (see the 2026-09-14 entry).
...
- **The same wrapper, the other half: "Reach Out" was reading past chats under a chat wrapper.**
  ...
  **Not changed, and worth stating as open:** outreach is the only reach-out pass with retrieval
  ...
  caveat as below: the evidence is its rate across real runs, not a test.
...
- **The revision pass was being told, last, to write a reply** (commit `01ea83b`). Reported as the pass steering
  ...
  register, and there is no way to tell those apart retroactively. The defect is a
  sampling tendency, so the evidence is its rate across real runs, not a test.
...
- **Worklog open threads never closed: the close site was wired to a subsystem that had
  not yet run on this box.** Reported from watching the Worklog tab (`open_threads()` only ever growing),
...
- **A row can now be refused rather than repaired.** The Training review tab had two
  ...
  **Ban from training / Un-ban** toggle on the edit bar (enabled by the wider `_can_ban`, so
  ...
  wander together. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **`[impression]` and the per-person user portrait — the persona digest, turned outward.**
  ...
  **No injection maturity gate:** the persona portrait gates on `_is_established` because it
  ...
  the real run-level seams with a scripted stub generator. Since exercised on the GPU box (see the
  2026-09-14 entry).
...
- **`[recollection]` — a fourth reflection-memory kind, so a revisit is visible to retrieval.**
  ...
  **RAG-only is structural, not a policy.** One op-log insert; no weights line, no ledger
  ...
  since exercised on the GPU box (see the 2026-09-14 entry).
...
- **A public OpenAI-compatible API, on its own port, that logs nothing.** External tools —
  ...
  **Non-streaming, deliberately and explicitly.** `stream:true` returns a 400 naming the
  ...
  no key warns at boot. Since exercised on the GPU box (see the 2026-09-14 entry); the
...
- **Persona reaches live chat as a standing portrait; the per-turn `[persona]` RAG channel
  ...
  *session-level* `chat.get("system_prompt")` (`training/dialogue_source.py`), never the
  ...
  encounter and ephemeral paths are unchanged. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **Map-reduce persona clustering wired into the reflection run; digest themes now carry
  ...
  **Expect the theme ranking to change materially on the first wired run**, since the blob
  ...
  dormant→active or the reverse. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- **Persona-digest clustering does not survive scale — measured, and a map-reduce
  ...
  **Wired as a DRY RUN only** (Sleep tab "Persona digest (dry)" → `digest_dryrun`): it writes
  ...
  `evidence.themes[].members` currently stores content strings only. Since exercised on the
  GPU box (see the 2026-09-14 entry).
...
- **LoRA scaling switched to rank-stabilized (`use_rslora=True`, fixed `alpha=4`) — `lora_r`
  ...
  **No migration.** peft records `use_rslora` in each `adapter_config.json` and recomputes γ
  ...
  original α/r == 1.0 scaling and load unchanged. Since exercised on the GPU box (see the 2026-09-14 entry).
...
- The 2026-07 collapse was **misattributed to sequential resumed-adapter stacking; corrected 2026-09.** The original account (memorization-grade verbatim fits sharpening the output distribution cycle over cycle, damage in the probability geometry rather than the tokens, visible only under sampling) did not survive the evidence: the collapse persisted after the switch to from-scratch training, so stacking was not the mechanism. The actual carrier was **RAG injection polluting the reflection cycle**, whose frozen targets then entered every subsequent build — so a fresh-from-scratch adapter inherited the pollution through the corpus, not the optimizer. Fixing the RAG mechanism resolved it. Consequences: the pure-stacking hypothesis is untested (never retried on the resumed method after the fix), the necessity of from-scratch rebuild is reopened, and pipeline degradation now requires fresh proof rather than being treated as established. See `AVA_OPEN_PROBLEMS.md → Cumulative Adapter Drift`.
...
- Validation is disabled, and (2026-09 stance) a blocking promotion gate is deliberately not wanted: the adapter is a disposable build artifact, acute breakage shows in the first live chat, and the deliverable is the process, not any adapter. UI skip-validation and judge-driven force-validation are both overridden by the shared switch. What is wanted from the probe is non-blocking per-build measurement, not a veto — see `AVA_OPEN_PROBLEMS.md → Validation`.

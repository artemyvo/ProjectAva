...
**Current state.** Ava is a research prototype built by one person on one GPU box. Every subsystem listed as built in [`documentation/AVA_STATUS.md`](documentation/AVA_STATUS.md) has been run on that box, but it is exercised by one operator against one corpus, and APIs, on-disk formats and defaults change without notice. It needs a Linux machine with a CUDA GPU (developed on an NVIDIA DGX Spark); setup is in [`DEPLOY.md`](DEPLOY.md). Commit hashes cited in the docs refer to the private development history.

...
# Crash supervision (2026-09-11). `_expected_stop` is True from _kill() until the next
# _launch(): a child exit seen while it is False is a crash, not a stop we asked for.
# `_last_exit` is the last such exit for GET /status; `_crash_relaunches` the times the
# reaper relaunched, for the crash-loop guard.
_expected_stop: bool = False
_last_exit: dict | None = None
_crash_relaunches: list = []
CRASH_RELAUNCH_MAX = 3          # relaunches this many crashes inside the window, then stays down
CRASH_RELAUNCH_WINDOW_S = 600.0
...
def _launch() -> None:
    ...
    global _proc, _start_time, _expected_stop
...
def _kill() -> None:
    ...
    global _proc, _start_time, _expected_stop
    _expected_stop = True       # the reaper must not read this exit as a crash
    if _proc is None:
        return
    try:
        _proc.kill()
...
_SIGNAL_HINTS = {
    "SIGKILL": "killed from outside — the kernel OOM killer (`dmesg -T | grep -i killed`) or a manual kill",
    "SIGSEGV": "native crash (segfault) — faulthandler's thread dump, if any, is right above in server.log",
    "SIGABRT": "native abort — a C++ assertion or a double-OpenMP clash; see the lines right above",
    "SIGBUS": "native crash (bus error)",
}


def _describe_exit(rc: int) -> tuple:
    """(human line, relaunch?) for a child's returncode. Negative = the signal that
    killed it: a crash, relaunch. Positive = the server exited on purpose (a model
    that will not load exits 1): relaunching repeats the failure, so stay down."""
    if rc < 0:
        try:
            name = signal.Signals(-rc).name
        except ValueError:
            name = f"signal {-rc}"
        return f"died on {name}: {_SIGNAL_HINTS.get(name, 'died on a signal')}", True
    return f"exited with code {rc} (a deliberate exit, not a crash — see the error above it)", False


def _crash_loop_tripped(times: list, now: float) -> bool:
    """Prune *times* to the window and say whether another relaunch is one too many."""
    times[:] = [t for t in times if now - t < CRASH_RELAUNCH_WINDOW_S]
    return len(times) >= CRASH_RELAUNCH_MAX


def _reaper() -> None:
    """Notice a child that died on its own, write what killed it into server.log, and
    relaunch it — the one thing a supervisor exists for and the one thing this one did
    not do: a crashed inference server stayed DOWN, silently, until someone pressed
    Restart (2026-09-11: three silent deaths in five days on the training box, each reaching the
    operator as "connection refused for a few minutes" — their own Restart plus the
    model load). server.log never says why, because a native crash prints no Python
    traceback and nobody printed the signal; this prints it, with the returncode's
    meaning (-9 SIGKILL is the OOM killer, -11 SIGSEGV a native crash). Stops we asked
    for (_kill, a job) are excluded via _expected_stop; a crash LOOP (CRASH_RELAUNCH_MAX
    inside CRASH_RELAUNCH_WINDOW_S) stops relaunching and says so, since the next
    launch would only reload the model for a minute to die the same way."""
    global _proc, _start_time, _last_exit
    while True:
        time.sleep(2.0)
        with _proc_lock:
            if _proc is None or _expected_stop:
                continue
            rc = _proc.poll()
            if rc is None:
                continue
            line, relaunch = _describe_exit(rc)
            uptime = round(time.monotonic() - _start_time) if _start_time else None
            _proc = None
            _start_time = None
            loop_note = ""
            if relaunch and _crash_loop_tripped(_crash_relaunches, time.monotonic()):
                relaunch = False
                loop_note = (f"; {CRASH_RELAUNCH_MAX} crash relaunches inside "
                             f"{int(CRASH_RELAUNCH_WINDOW_S)} s — not relaunching again "
                             f"(POST /restart to try by hand)")
            msg = (f"inference server {line} after {uptime} s up{loop_note}; "
                   + ("relaunching" if relaunch else "staying down"))
            _last_exit = {"returncode": rc, "at": time.time(), "uptime_s": uptime,
                          "detail": line, "relaunched": relaunch}
            _log_server(msg)
            print(f"[watchdog] {msg}", flush=True)
            if relaunch:
                _crash_relaunches.append(time.monotonic())
                _launch()
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/status":
            ...
            self._json(200, {
                ...
                "job": job,
                # The last exit the reaper saw (None if never): {returncode, at,
                # uptime_s, detail, relaunched} — how a client can say "the server
                # crashed at 03:12 on SIGSEGV" instead of "disconnected".
...
def main() -> None:
    ...
    with _proc_lock:
        _launch()
    threading.Thread(target=_reaper, name="reaper", daemon=True).start()

...
SCHEMA: list[Tab] = [
    ...
    Tab(
        ...
        [
            Field("deliberation.mode", "Mode", "combo", "shadow",
                  choices=["off", "shadow", "sole"],
                  tooltip="off: no autonomous deliberation (the Worklog tab's button still "
                          "works, dry-run). shadow (default): the executive runs as an idle "
                          "job AND the per-drive timers keep firing, so her choices can be "
                          "compared with theirs in the Activity tab. sole: the executive is "
                          "the only chooser — the drives' own timers are not registered; "

...
# The running event loop, captured by _main_async so a blocking job on the executor thread
# can schedule a coroutine onto it (the deliberation executive starting a revisit run).
_event_loop: Optional[asyncio.AbstractEventLoop] = None


async def _send(ws, msg: dict) -> None:
    # A headless caller (the deliberation executive starting a reflection run with no
    # client attached) passes ws=None: the run proceeds, its events reach the run store
    # and the activity journal as always, and there is simply nobody to push them to.
    if ws is None:
        return
...
async def handle_load(ws, msg: dict) -> None:
    if _runtime.loading:
        await _send(ws, {"type": "error", "message": "The server is still loading its startup "
                                                     "model; wait for it to finish."})
        return
...
async def handle_status(ws) -> None:
    await _send(ws, {
        ...
        # The startup auto-load is still running (the socket is up before the model
        # is, since 2026-09-11): the client shows "loading" and waits for `loaded`.
...
async def _announce_loaded() -> None:
    """Tell the connected client (if any) that the startup load landed — the same
    `loaded` message handle_load answers with, so BackendClient flips `is_loaded` and
    the Chat tab unlocks without a reconnect."""
    ws = _active_ws
    if ws is None:
        return
    try:
...
async def _main_async(host: str, port: int, startup_load=None) -> None:
    ...
    #
    # open_timeout=None extends that decision to the OPENING HANDSHAKE (2026-09-11).
    # ping_timeout=None protects a socket that is already up; a client CONNECTING while
    # the loop is starved still had to complete the HTTP upgrade inside the default 10 s,
    # and could not — the assoc feed (core/assoc_bridge.run_feed_blocking: lemmatization,
    # codebook, dedup, PageRank — minutes of pure-Python work that, unlike a CUDA-bound
    # generation, never releases the GIL) fires 5 min into idle, so a user returning to
    # the box was met with "timed out during opening handshake" and read it as a refusal.
    # The accept still happens; it just waits for a loop slice. The client bounds its own
    # side (backend_client.connect), so a dead peer still fails there.
    ...
    async with websockets.asyncio.server.serve(
        handle_client, host, port, max_size=None, ping_timeout=None, open_timeout=None
    ):
        ...
        # The startup auto-load, on the executor, WITH the socket already listening
        # (2026-09-11). It used to run before asyncio.run(), so every relaunch — a git
        # pull, a Restart, a crash — REFUSED connections for the whole load: a minute or
        # more on this box, "[Errno 61] Connection refused" in the client, which is a
        # dead server's symptom for a server that was merely booting. Now `status`
        # answers `loading: true` meanwhile, the idle jobs wait on model_loaded(), and
        # a client that connected early is told `loaded` when the model lands. A load
        # that fails still exits the process (rc 1, a deliberate exit the watchdog
        # reports rather than relaunches), as it always did.
        if startup_load is not None:
            ok = await asyncio.get_running_loop().run_in_executor(_executor, startup_load)
            if not ok:
                raise SystemExit(1)
            await _announce_loaded()
...
def main() -> None:
    ...
    # A native crash — SIGSEGV/SIGABRT inside torch, faiss, bitsandbytes, xformers —
    # kills this process with NO Python traceback: server.log simply ends, and the next
    # thing in it is a relaunch banner. Three such silent deaths in five days (2026-09-07
    # → 09-11), each read by the operator as "connection refused for a few minutes".
    # Dump every thread's Python stack on those signals, to fd 2 directly (the watchdog
    # merges stderr into server.log; the stdout tee below wraps the Python-level
    # objects, not the descriptor), so the next one names the call it died in. SIGKILL
    # (the kernel OOM killer) cannot be caught — the watchdog reports that returncode.
    ...
    # (The deliberation executive is wired further down, beside the idle-job
    # registrations, because its dispatch map IS those jobs' bodies.)
    ...
    # See core.modules.
    # The associative library (assoc/, ASSOCIATIVE_MEMORY.md): fed from the same
    # protocols the passes above write, read by the `assoc_fetch` workbench module and —
    # under `assoc.enabled` — by stage 1 of a chat turn. Paths only; the model comes per
    # call through the reflect seam.
    assoc_bridge.configure(
        chats_dirs=[_CHATS_DIR, _SERVER_DIR.parent / "data" / "archive" / "chats"],
        til_snippets_dir=_SERVER_DIR.parent / "data" / "til" / "snippets",
        root=_SERVER_DIR.parent / "data" / "assoc",
        prompts_dir=_PROMPTS_DIR,
        load_config=_load_server_config,
        # The aha job's seams: open asks → needs, the writer's surface stamp, the reflect
        # seam for the judge + opener, and outreach's own session writer + addressee so an
        # aha reach-out is written exactly as an outreach one.
        memory_dir=_MEMORY_DIR, get_reflection_writer=_get_reflection_writer,
        get_rag=_get_rag, make_sync_reflect_generate=generation._make_sync_reflect_generate,
        write_session=outreach._write_outreach_session, target_user=outreach._target_user,
        model_loaded=lambda: _runtime.model is not None,
        # The adapter's name, recorded on every witness protocol so a refused document is
        # re-witnessed once per new adapter (assoc_bridge.witness_model_id).
        model_name=lambda: (Path(_runtime.adapter_id).name if _runtime.adapter_id else (_runtime.model_id or "")),
    )
    modules.configure(
        get_rag=_get_rag,
        make_sync_reflect_generate=generation._make_sync_reflect_generate,
    ...
    # Read by the deliberation executive (core.deliberation) and shown in the Worklog tab.
    # See core.worklog.
    from core import worklog as _worklog
    ...
    #
    # The DRIVES — the jobs that are choices rather than upkeep — are defined ONCE here as
    # bodies and handed to two consumers: their own IdleJob (independent clock) and the
    # deliberation executive's dispatch map (core.deliberation), so "wander" means the same
    # thing whether a timer or her own decision triggered it. Under `deliberation.mode:
    # "sole"` the timers are not registered and the executive is the only chooser.
    def _run_outreach() -> dict:
        return outreach.run_outreach_decision_blocking(on_question=_outreach_on_question)

    def _run_synthesis() -> dict:
        return synthesis.run_synthesis_blocking(on_stage=_synthesis_on_stage)

    def _run_checkin() -> dict:
        return checkin.run_checkin_sweep_blocking(on_stage=_checkin_on_stage)

    def _run_wander() -> dict:
        # The IdleJob carries the budget as a `ready` gate; the executive has no gate stage,
        # so the same precondition is checked here and reported rather than overspent.
        if til_wander._wander_budget_available() < 1:
    ...
    def _run_revisit() -> dict:
        ...
        # return; it queues behind the deliberation that chose it and holds
        # `_reflection_run_active` (external_busy) for its duration, so the scheduler
        # stays clear. ws=None: no client to push events to (see _send).
        if _event_loop is None:
            return {"skipped": "no_event_loop"}
        msg = {"revisit": True, "source": "deliberation",
    ...
    _drives = {
        ...
        "wander": _run_wander,
        "assoc_aha": assoc_bridge.run_aha_blocking,
        "assoc_pivot": assoc_bridge.run_pivot_blocking,
    }
    _delib_cfg = deliberation.settings(config)
    _timers_on = _delib_cfg["mode"] != "sole"
    if not _timers_on:
        print("[deliberation] mode=sole — the drives (outreach/synthesis/checkin/wander/"
              "aha/pivot) run only when she chooses them; their timers are not registered",
              flush=True)

    def _register_drive(job: "idle_scheduler.IdleJob") -> None:
        if _timers_on or job.name not in deliberation.CHOSEN_JOBS:
            idle_scheduler.register(job)

    _register_drive(idle_scheduler.IdleJob(
        name="outreach", interval_s=3600.0,
        run=_drives["outreach"],
        consumed=_outreach_consumed_interval,
        describe=_describe_outreach,
    ))
    _register_drive(idle_scheduler.IdleJob(
        name="synthesis", interval_s=3600.0,
        run=_drives["synthesis"],
    ...
    # Associative-library upkeep: `ava_import.sync` over the live corpus (a stat per file;
    ...
    # The aha (ASSOCIATIVE_MEMORY.md §4) on the reach-out surface: her open asks are the
    # library's standing needs, what the feed brought in since the last wake is the
    # stimulus, `aha()` is arithmetic and the judge a thinking-on pass (minutes — one per
    # wake by default), and a `connect`/`satisfies` verdict becomes an opener written the
    # way outreach writes one, behind the same reach-out gate and backoff. A cheap bail
    # (no needs, nothing fed, no candidate) retries next poll; a judgement spends the hour.
    # The library's own witness over her chats, replacing the imported `chat_facts` protocol
    # newest first, `assoc.witness_per_wake` per wake (~5 min per chat at 4.5 tok/s). Skips
    # without spending its interval when no model is loaded; `assoc.witness: false` turns
    # it off. Runs on the default idle window like the reach-out jobs — it holds the GPU
    # for minutes.
    idle_scheduler.register(idle_scheduler.IdleJob(
        name="assoc_witness", interval_s=3600.0,
        run=assoc_bridge.run_witness_blocking,
        consumed=assoc_bridge.witness_consumed,
        describe=assoc_bridge.describe_witness,
    ))
    # The pivot (ASSOCIATIVE_MEMORY.md §6): from the newest conversation, a word with another
    # sense on record; one thinking-on creative pass decides raise (an outreach behind the
    # gate) / keep (a worklog note) / nothing. `assoc.pivot: false` turns it off;
    # `assoc.pivot_bridges` adds `sound` / `root` to the default `sense`.
    _register_drive(idle_scheduler.IdleJob(
        name="assoc_pivot", interval_s=3600.0,
        run=_drives["assoc_pivot"],
        consumed=assoc_bridge.pivot_consumed,
        describe=assoc_bridge.describe_pivot,
    ))
    _register_drive(idle_scheduler.IdleJob(
        name="assoc_aha", interval_s=3600.0,
        run=_drives["assoc_aha"],
        consumed=assoc_bridge.aha_consumed,
        describe=assoc_bridge.describe_aha,
    ))
    ...
    deliberation.configure(
        ...
        host_busy=_host_busy,
        dispatch={
            "wander": _drives["wander"],
            "synthesize": _drives["synthesis"],
            "reach_out": _drives["outreach"],
            "check_in": _drives["checkin"],
    ...
    if _delib_cfg["mode"] != "off":
        idle_scheduler.register(idle_scheduler.IdleJob(
            name="deliberation", interval_s=float(_delib_cfg["interval_s"]),
            run=deliberation.run_deliberation_job,
            consumed=deliberation.consumed,
            describe=deliberation.describe,
        ))
        print(f"[deliberation] executive on: mode={_delib_cfg['mode']} "
              f"interval={int(_delib_cfg['interval_s'])}s execute={_delib_cfg['execute']}",
              flush=True)
    ...
    if model_id:
        ...
        def startup_load() -> bool:
            """The startup auto-load — run by _main_async on the executor thread once
            the WebSocket is listening (see the note there). Returns False on failure;
            the caller exits the process. Its locals die with this frame, which is what
            the old inline block had to `del` its way to: main()'s frame never exits,
            and a model bound there is pinned beside _runtime for the life of the
            process — two copies to fit on the card at every clean-base swap."""
            if adapter_id:

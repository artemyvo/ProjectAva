...
def _paths(server_dir: Optional[Path] = None) -> dict[str, Path]:
    ...
    return {
        ...
        # The associative library (core.assoc_bridge): derived from the chats + TIL
        # protocols and rebuilt by the feed job, so it goes with the regenerable state — a
        # library left standing would inject facts from conversations the wipe removed.
        "assoc": server / "data" / "assoc",
...
def wipe_reflection_state(server_dir: Optional[Path] = None) -> dict:
    ...
    for key in ("runs", "staging", "persona", "users", "wander", "prompt", "assoc"):

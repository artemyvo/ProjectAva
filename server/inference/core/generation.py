...
def _fetch_facts_block_sync(user_message: str, speaker: str, conversation: list) -> dict:
    ...
    # The associative library (`core.assoc_bridge`, ASSOCIATIVE_MEMORY.md) takes the turn
    # under `assoc.enabled` (on by default since 2026-09-10): same stage, same result
    # shape, a different store — three grains, cross-lingual cells, the activation state
    # warmed by what was injected. A library that is not there yet (no build before the
    # first feed, the package or its embedder missing) falls THROUGH to the tree channel
    # rather than costing the turn its facts. Every path inside is caught like the tree's.
    try:
        from core import assoc_bridge
        if assoc_bridge.config()["enabled"]:
            with activity_log.pass_context("assoc_fetch"):
                fetched = assoc_bridge.fetch_block(
                    user_message, speaker, list(conversation),
                    reflect=_make_sync_reflect_generate(_get_rag()),
                    template=_load_facts_block_template())
            if fetched.get("skipped") not in ("no_build", "no_library"):
                return fetched
    except Exception as e:
        return {"text": "", "skipped": "error", "error": str(e), "channel": "assoc"}
    try:
...
async def handle_generate(ws, msg: dict, msg_queue: asyncio.Queue) -> None:
    ...
    if success:
        ...
        # What the associative library injected was USED: an access on those nodes (its
        # activation state, §3.2 of ASSOCIATIVE_MEMORY.md). A sqlite write; off the loop.
        if fetched.get("channel") == "assoc" and fetched.get("assoc_ids"):
            try:
                from core import assoc_bridge
                await asyncio.get_running_loop().run_in_executor(
                    None, assoc_bridge.touch_after_turn, fetched)
            except Exception:
                pass
